...
type
  ...
  published
    ...
    procedure Test_FillUnChecked_Index;
    procedure Test_FillUnChecked_Index_Count;
...
begin
  ...
  try
    ...
    try
      ...
      { 使用默认构造（隐式使用RTL分配器） }
      LArray := specialize TArray<Integer>.Create;
      AssertNotNull('Default Create should work', LArray);
      AssertNotNull('Default Create should use RTL allocator', LArray.GetAllocator);
...
begin
  ...
  try
    ...
    try
      ...
      { 显式传入RTL分配器（避免传入nil） }
      LArray := specialize TArray<Integer>.Create(GetRtlAllocator(), LTestData);
      AssertNotNull('Create with rtl allocator should work', LArray);
      AssertNotNull('Create with rtl allocator should use RTL allocator', LArray.GetAllocator);
      AssertTrue('Create with nil allocator should use RTL allocator', LArray.GetAllocator = GetRtlAllocator());
      AssertTrue('Array should store provided data', LArray.GetData = LTestData);
      AssertTrue('Array should be empty', LArray.IsEmpty);
    finally
  ...
  try
    ...
    try
      ...
      try
        ...
        { 使用来源构造 + 默认分配器（避免传入nil） - 非托管类型 }
        LTargetArray.Free;
        LTargetArray := specialize TArray<Integer>.Create(LSourceArray);
        AssertNotNull('Create with default allocator should work', LTargetArray);
        AssertNotNull('Create with default allocator should use RTL allocator', LTargetArray.GetAllocator);
    ...
    try
      ...
      try
        ...
        { 使用来源构造 + 默认分配器（避免传入nil） - 托管类型 }
        LTargetStringArray.Free;
        LTargetStringArray := specialize TArray<String>.Create(LSourceStringArray);
        AssertNotNull('Create with default allocator should work (managed)', LTargetStringArray);
        AssertNotNull('Create with default allocator should use RTL allocator (managed)', LTargetStringArray.GetAllocator);
...
procedure TTestCase_Array.Test_FillUnChecked;
var
  A: specialize TArray<Integer>;
  i: SizeInt;
begin
  A := specialize TArray<Integer>.Create(6);
  try
    // 先用 Fill 建立基线
    A.Fill(0, 6, 7);
    for i := 0 to A.GetCount - 1 do
      AssertEquals('baseline fill', 7, A[i]);

    // 用 UnChecked 改写[1..4]
    A.FillUnChecked(1, 4, 9);
    AssertEquals(7, A[0]);
    for i := 1 to 4 do
      AssertEquals('unchecked fill range', 9, A[i]);
    AssertEquals(7, A[5]);
  finally
    A.Free;
  end;
...
procedure TTestCase_Array.Test_FillUnChecked_Index;
var
  A: specialize TArray<Integer>;
  i: SizeInt;
begin
  A := specialize TArray<Integer>.Create([1,2,3,4,5]);
  try
    A.FillUnChecked(2, 3, 42);
    AssertEquals(1, A[0]);
    AssertEquals(2, A[1]);
    for i := 2 to 4 do
      AssertEquals(42, A[i]);
...
procedure TTestCase_Array.Test_FillUnChecked_Index_Count;
var
  A: specialize TArray<String>;
  i: SizeInt;
begin
  // 托管类型：验证行为一致性
  A := specialize TArray<String>.Create(['a','b','c','d']);
  try
    A.FillUnChecked(1, 2, 'X');
    AssertEquals('a', A[0]);
    for i := 1 to 2 do
      AssertEquals('X', A[i]);
    AssertEquals('d', A[3]);
