...
## Research Findings
- 最新一轮在 `SIMD-B21(candidate)` 收口后转审 `SIMD-B20(candidate)`，确认 backlog 里残留的 B20 已经不是“待实现自动化”，而是单纯的真相源漂移：
  - 现态脚本链已完整存在：
    ...
    - `tests/fafafa.core.simd/run_windows_b07_closeout_via_github_actions.sh`
  ...
  - fresh release 复验结果：
    - `FAFAFA_BUILD_MODE=Release bash tests/fafafa.core.simd/verify_windows_b07_evidence.sh`
      - PASS，`[EVIDENCE] OK: .../tests/fafafa.core.simd/logs/windows_b07_gate.log`
    - `FAFAFA_BUILD_MODE=Release bash tests/fafafa.core.simd/BuildOrTest.sh finalize-win-evidence`
      - PASS，`[CLOSEOUT] OK: .../tests/fafafa.core.simd/logs/windows_b07_closeout_summary.md`
    - `FAFAFA_BUILD_MODE=Release bash tests/fafafa.core.simd/BuildOrTest.sh freeze-status`
      - FAIL，但失败点不是 automation 缺口，而是 freshness 守卫：
        - `linux_sources_not_newer_than_gate`
        - `linux_sources_not_newer_than_windows_evidence`
      - 具体含义：当前源码已有 2026-03-27 新改动，而 canonical Linux/Windows 归档证据仍停在 2026-03-24；因此 freeze 继续要求 fresh evidence，这符合预期
  - 结论：
    - `SIMD-B20(candidate)` 应视为 completed
    - 当前不应再把 “Windows evidence automation” 当作 active SIMD candidate
    - 若要把 `freeze-status` 拉回绿，下一步应生成新的 2026-03-27+ Linux/Windows 证据批次，而不是继续修 B20 脚本
- 最新一轮继续沿真实 RVV opcode-ready broad suite 深挖后，确认 summary 里遗留的 generic shuffle underclaim 怀疑点在当前 worktree 已不再复现，真正暴露出的生产问题是 `RISCVV_ASSEMBLY` 注册层的宽 rounding stale wiring：
  - x86 host fallback spot-check：
    ...
    - 当前结果直接是 PASS，说明 generic `scShuffle` underclaim testcase 在现态没有继续打红
  - broad RVV native lane 在 testcase 守卫和 runtime 前置条件补真之后，fresh red 收敛到：
  ...
  - 根因确认：
    - `tests/fafafa.core.simd/fafafa.core.simd.dispatchapi.testcase.pas` / `tests/fafafa.core.simd/fafafa.core.simd.publicabi.testcase.pas` 之前还有一批 native broad contract 依赖测试单元不可见的 `RISCVV_ASSEMBLY` 语义或默认 `vectorAsm=False` 初态
    - 把这些 broad contract 收紧到测试可见的 `FAFAFA_SIMD_TEST_RISCVV_ASM_COMPILED`，并显式 `SetVectorAsmEnabled(True)` 后，suite 才第一次真正观测到 native asm 注册态
    - 真正的生产缺陷在 `src/fafafa.core.simd.riscvv.register.inc`：
      - `RISCVV_ASSEMBLY` 分支仍把 `Ceil/Floor/Round/Trunc` 的 `F32x16/F32x8/F64x2/F64x4/F64x8` 槽位绑到 `Scalar*`
      - 但 `src/fafafa.core.simd.riscvv.pas` 已提供对应 `RISCVV*` asm implementation，因此这不是 helper 缺失，而是 register wiring 过时
  - 最小修复方式：
    - `src/fafafa.core.simd.riscvv.register.inc` 现已把上述宽 rounding 槽位在 `RISCVV_ASSEMBLY` 下改回 `RISCVV*`
    - `tests/fafafa.core.simd/fafafa.core.simd.test.lpr` 同步挂入 `TTestCase_RISCVVMaskedOpsContract` 与 `TTestCase_RISCVFallbackDispatchContract`，避免 testcase/register/runner manifest 继续漂移
  - fresh release 复验：
    - broad RVV lane：
...
- 最新一轮继续沿 `RVV asm + OPCODE_READY` 的 fresh `TTestCase_PublicAbi` red 深挖后，确认这次也不是新的生产 dispatch/public-ABI 缺陷，而是 public ABI testcase 自己的 early-exit cleanup 污染：
  - 稳定 red 命令是：
  ...
  - 第一眼看起来像 `Test_PublicApi_DataPlane_Parity` 在 `MemEqual(facade)` 上 AV，但加最小诊断后确认 failure 时：
    - `VectorAsm=False`
    - `CurrentBackend=Scalar`
    - `Dispatch.MemEqual=nil`
    - `Scalar.MemEqual=nil`
  - 再往前追溯后，真正脏化点落在 `tests/fafafa.core.simd/fafafa.core.simd.publicabi.testcase.pas` 的 `Test_PublicApi_RollbackRestore_Success_Preserves_ForcedSelection`
    - 这条测试在 RVV lane 只有 `Scalar + RISCVV` 两个 dispatchable backend 时会 early-exit
    - 但旧 `finally` 仍无条件执行 `RegisterBackend(GPublicAbiHookRollbackForceSuccessTarget, GPublicAbiHookRollbackForceSuccessTargetTable)`
    - 此时 global target 仍是默认 `sbScalar`，target table 仍是 `Default(TSimdDispatchTable)`，从而把 scalar registered/current snapshot 冲成零值表
    - 后继的 `DataPlane_Parity` 才会在最先调用到的 `MemEqual(facade)` 处崩掉
  - 最小修复不是改生产实现，而是把 public ABI testcase 收紧到与 `DispatchAPI` 对称：
    - 入口显式清理 rollback-force-success globals
    - 新增 `LTargetTableCaptured`
    - 只有真正捕获过 requested backend 原表时才执行 restore 回写
  - 这轮同时证伪了上一轮错误假设：
    - `src/fafafa.core.simd.public_abi.impl.inc` 对 `CPURISCV64` direct-bind 的收紧不是根因
    - 现已恢复 64-bit direct-bind 范围重新包含 `CPURISCV64`
  - fresh release 复验：
    - RVV `TTestCase_PublicAbi`：PASS，`[RVV-LANE] PASS`
    - RVV `TTestCase_DispatchAPI`：PASS，`[RVV-LANE] PASS`
- 最新一轮继续沿 RVV fallback `TTestCase_DispatchAPI` broad failure 深挖后，确认这次不是新的生产 dispatch/capability 缺陷，而是一条测试自身的 cleanup 污染：
  - historical riscv64 fallback lane 里从 `Test_VecI64x2_DispatchAssigned_And_Parity` 开始的大面积 `should be assigned` / AV，并不是 scalar/current dispatch 真缺 wide slots
  - fresh red 已把第一处脏化点缩到 `tests/fafafa.core.simd/fafafa.core.simd.dispatchapi.testcase.pas` 的 `Test_TrySetActiveBackend_RollbackRestore_Success_Preserves_ForcedSelection`
  - 根因是这条测试在 fallback 早退路径里仍无条件执行：
    - `RegisterBackend(GDispatchHookRollbackForceSuccessTarget, GDispatchHookRollbackForceSuccessTargetTable)`
    - 但此时 global target 仍是默认 `sbScalar`，target table 仍是 `Default(TSimdDispatchTable)`
    - 结果就是把 scalar 注册表直接重写成零值表，后续 suite 再读 current dispatch 就会观察到 representative wide slots 被打空
  - 最小修复不是改生产实现，而是把 restore 收紧到“只有真正捕获过 requested backend 原表时才允许回写”，并在入口显式清理 rollback-force-success globals
  - fresh release 复验：
    - host `TTestCase_DispatchAPI`：PASS
    - riscv64 fallback `TTestCase_RISCVFallbackDispatchContract`：PASS
    - riscv64 fallback `TTestCase_DispatchAPI`：PASS
...
- 最新一轮继续深审 `TrySetActiveBackend(...)` failure rollback / restore 路径后，又确认一条新的真实 control-plane drift：
  ...
  - 结果是只要调用前已经 forced 在 backend A、尝试切到 backend B、且 hook 先让 `B` 失败进入 rollback：
    - failure rollback 本身已经会走回 previous forced backend restore 路径，这本身不是问题
    - 可如果 dispatch-changed hook 在最后一次 restore callback 里再 nested `SetActiveBackend(sbScalar)`，旧实现不会在返回前把 control-plane intent 再次收回来
    - 所以 return-time current/public ABI 会再次停在 stale scalar forced fallback，而不是 previous forced backend A
  - 为了先把合同打红，本轮补了两条新的 deterministic regression：
    - `tests/fafafa.core.simd/fafafa.core.simd.dispatchapi.testcase.pas` 新增 `Test_TrySetActiveBackend_RollbackRestore_LateForce_Preserves_PreviousForcedBackend`
    - `tests/fafafa.core.simd/fafafa.core.simd.publicabi.testcase.pas` 新增 `Test_PublicApi_RollbackRestore_LateForce_Preserves_PreviousForcedBackend`
  - fresh red 证据：
    - `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-rollback-lateforce-red-20260323 bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_DispatchAPI,TTestCase_PublicAbi`：FAIL
    - 失败点直接命中：
  ...
  - 最小修复方式：
    - 将 `src/fafafa.core.simd.dispatch.pas` 的 `TrySetActiveBackendInternal(...)` 收紧为：
      - previous-forced rollback restore 的 `InitializeDispatch` 完成后立即检查 `g_BackendForced/g_ForcedBackend`
      - 若 hook 再次改写 control-plane mode，则恢复 `LPreviousForcedBackend` 并再做一次 `InitializeDispatch`
    - 这样 `TrySetActiveBackend(...)` 仍保留 Phase 49/50/51/52 已验证的 failure rollback 语义，但不会在最后一跳 restore callback 里再次被 late scalar force 劫持
  - fresh green / 复验证据：
    - `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-rollback-lateforce-green-20260323 bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_DispatchAPI,TTestCase_PublicAbi`：PASS，`[LEAK] OK`
    - `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-rollback-lateforce-check-20260323 bash tests/fafafa.core.simd/BuildOrTest.sh check`：PASS
    - `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-rollback-lateforce-gate-20260323 bash tests/fafafa.core.simd/BuildOrTest.sh gate`：PASS，最终 `[GATE] OK`，run-all summary 时间 `2026-03-23 20:01:24`
- 最新一轮继续深审 `RegisterBackend(...)` / rebuild-hook 嵌套路径后，又确认一条新的真实 control-plane drift：
  - `src/fafafa.core.simd.dispatch.pas` 的 `RegisterBackend(...)` 旧实现虽然已经会 canonicalize table identity / priority / canonical text，并在需要时做一次 `InitializeDispatch`
  - 但 `DoInitializeDispatch -> NotifyDispatchChangedHooks` 允许 dispatch-changed hook 在 `RegisterBackend(...)` 返回前 nested `SetActiveBackend(sbScalar)`
  - 结果是只要调用前处于 automatic mode，且 automatic best backend 不是 scalar：
    - `RegisterBackend(...)` 中间那次 reinit 本来已经能选回 automatic best backend，这本身不是问题
    - 可旧实现不会在返回前恢复 pre-call control-plane mode
    - 所以 hook 只要在通知阶段 re-force scalar，函数 return-time active/public ABI 就会停在 stale scalar forced fallback，而不是 automatic best backend
  - 为了先把合同打红，本轮补了两条新的 deterministic regression：
    - `tests/fafafa.core.simd/fafafa.core.simd.dispatchapi.testcase.pas` 新增 `Test_RegisterBackend_HookLateForce_Restores_AutomaticBackend`
    - `tests/fafafa.core.simd/fafafa.core.simd.publicabi.testcase.pas` 新增 `Test_PublicApi_RegisterBackend_HookLateForce_Restores_AutomaticBackend`
  - fresh red 证据：
    - `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-register-lateforce-red-20260323 bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_DispatchAPI,TTestCase_PublicAbi`：FAIL
    - 失败点直接命中：
  ...
  - 最小修复方式：
    - 将 `src/fafafa.core.simd.dispatch.pas` 的 `RegisterBackend(...)` 收紧为：
      - 入口保存 `LPreviousBackendForced/LPreviousForcedBackend`
      - 首次 `InitializeDispatch` 完成后检查 hook 是否改写了 `g_BackendForced/g_ForcedBackend`
      - 若 control-plane mode 与 pre-call intent 不一致，则恢复 pre-call intent 并再做一次 `InitializeDispatch`
    - 这样 `RegisterBackend(...)` 仍会发布 snapshot 并通知 hook，但不会在 automatic caller 语义下把 return-time state 劫持成 scalar forced fallback
  - fresh green / 复验证据：
    - `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-register-lateforce-green-20260323 bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_DispatchAPI,TTestCase_PublicAbi`：PASS，`[LEAK] OK`
    - `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-register-lateforce-check-20260323 bash tests/fafafa.core.simd/BuildOrTest.sh check`：PASS
    - `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-register-lateforce-gate-20260323 bash tests/fafafa.core.simd/BuildOrTest.sh gate`：PASS，最终 `[GATE] OK`，run-all summary 时间 `2026-03-23 19:30:01`
...
- 最新一轮继续深审 raw registered snapshot / dispatch contract 后，又确认一条新的真实 metadata drift：
  ...
  - 结果是只要 caller 用空文本重注册某个 backend：
    - `GetBackendInfo(backend)` 会在读取时 fallback 出非空 canonical 文本
    - `GetCurrentBackendInfo` 与 `GetBackendOps(backend)` 现在也会各自兜底
    - 可 `TryGetRegisteredBackendDispatchTable(backend, ...)` 仍会把 raw published snapshot 里的空文本直接露出来
  - 这说明 shared dispatch metadata source 本身还留着空文本，helper 层虽然被补绿了，但底层 registered snapshot 仍然不是单一真相
  - 为了先把合同打红，本轮补了一条最小 deterministic regression：
    ...
    - 随后直接调用 `TryGetRegisteredBackendDispatchTable(LBackend, LReloadedTable)`，断言 raw registered snapshot 仍必须返回 canonical backend id、非空文本，并与 `GetBackendInfo(LBackend)` 对齐，同时继续保留当前 `Available/Capabilities`
  - fresh red 证据：
    - `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-registeredtable-text-red-20260322 bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_DispatchAPI`：FAIL
    - 失败点直接命中：
  ...
  - 最小修复方式：
    - 将 canonical text 收口回真正的 shared dispatch source，而不是继续在 helper 层加新补丁
    - `src/fafafa.core.simd.dispatch.pas` 的 `RegisterBackend(...)` 现在在发布 immutable snapshot 之前：
      - 如果 `BackendInfo.Name=''`，就写回 `DefaultBackendName(backend)`
      - 如果 `BackendInfo.Description=''`，就写回 `DefaultBackendDescription(backend)`
    - 这样 raw registered snapshot、current snapshot、adapter helper 与 public ABI text getter 都重新共享同一份 canonical backend 文本
  - fresh green / 复验证据：
    - `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-registeredtable-text-dispatchapi-green-20260322 bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_DispatchAPI`：PASS，`[LEAK] OK`
    - `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-registeredtable-text-dispatchslots-green-20260322 bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_DispatchAllSlots`：PASS，`[LEAK] OK`
    - `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-registeredtable-text-publicabi-green-20260322 bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_PublicAbi`：PASS，`[LEAK] OK`
    - `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-registeredtable-text-check-20260322 bash tests/fafafa.core.simd/BuildOrTest.sh check`：PASS
    - `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-registeredtable-text-gate-20260322 bash tests/fafafa.core.simd/BuildOrTest.sh gate`：PASS，最终 `[GATE] OK`，run-all summary 时间 `2026-03-22 03:54:33`
...
- 最新一轮继续深审 registered-view helper 并发合同后，又确认一条新的真实 mixed-snapshot：
  - `src/fafafa.core.simd.framework.impl.inc` 的 `GetRegisteredBackendList` 之前采用双遍 live 扫描：
    ...
    - 第二遍再分配后的 `Result[...]` 里逐项填充 backend
  - 这在“previously-unregistered backend 首次注册”的并发窗口里会暴露 impossible list snapshot：数组长度来自第一次观察点，而内容来自第二次观察点
  - 结果是 reader 会看到根本不属于任何真实 registered-state 的 backend list，例如列表里出现 `8`，却缺失更早已经先注册的 `9`
  - 为了先把合同打红，本轮新增独立并发回归：
    - `tests/fafafa.core.simd/fafafa.core.simd.concurrent.testcase.pas` 新增 `TTestCase_SimdConcurrentRegistration`
    - 新增 `Test_Concurrent_RegisteredBackendList_FirstRegistration_ReadConsistency`
    - writer `TBackendFirstRegisterSequenceWorker` 以降序逐个把 previously-unregistered backends 首次注册进 binary
    - reader `TRegisteredBackendListReadWorker` 持续断言 `GetRegisteredBackendList` 只能等于 base state、或按注册顺序逐步扩展出来的若干合法前缀态
    - `tests/fafafa.core.simd/fafafa.core.simd.test.lpr` 也同步接入这条 suite，确保 `--suite=TTestCase_SimdConcurrentRegistration` 在主 runner 可达
  - fresh red 证据：
    - `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-registeredlist-red-20260322 bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_SimdConcurrentRegistration`：FAIL
    - 失败点直接命中 impossible list snapshot：
  ...
  - 最小修复方式：
    - 将 `src/fafafa.core.simd.framework.impl.inc` 的 `GetRegisteredBackendList` 改为：
      - 先按 backend 总数一次性预分配上限容量
      - 单遍扫描里边判断 registered、边写入 `Result`
      - 扫描结束后再 `SetLength(Result, LCount)` shrink 到真实长度
    - 这样单个 list helper 调用不再把 count/fill 拆成两个独立 observation point
  - fresh green / 复验证据：
    - `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-registeredlist-green-20260322 bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_SimdConcurrentRegistration`：PASS，`[LEAK] OK`
    - `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-framework-recheck-20260322 bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_SimdConcurrentFramework`：PASS，`[LEAK] OK`
    - `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-registeredlist-check-20260322 bash tests/fafafa.core.simd/BuildOrTest.sh check`：PASS
    - `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-registeredlist-gate-20260322 bash tests/fafafa.core.simd/BuildOrTest.sh gate`：PASS，最终 `[GATE] OK`，run-all summary 时间 `2026-03-22 02:30:17`
  - 这轮还确认了一个测试组织约束：
    - `TTestCase_SimdConcurrentRegistration` 会永久把 previously-unregistered backend 注册进当前进程，属于 stateful suite
    - 因此它要保持独立 suite，并尽量放在 runner handle 顺序的末尾，避免污染其他并发合同测试
...
| `GetSimdPublicApi` 文档与设计承诺的是“绑定后直调”的 public API snapshot，但 `RebindSimdPublicApi` 之前实际把 `MemEqual/MemFindByte/...` 等 data-plane 字段统一写成 `@PublicAbi*` wrapper，导致 backend 切换后 fresh table 的 metadata 会刷新，data-plane 函数指针却不跟着切换 | 已在 `tests/fafafa.core.simd/fafafa.core.simd.publicabi.testcase.pas` 新增 `Test_PublicApi_Table_Rebinds_DataPlane_FunctionPointers_AfterBackendSwitch` 打红；fresh red `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-publicapi-bind-red2-20260322 bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_PublicAbi` 直接命中 `Fresh public API table should publish rebound function pointer for MemEqual`。随后把 `src/fafafa.core.simd.public_abi.impl.inc` 收紧为：在 `CPUX86_64/CPUAARCH64/CPURISCV64` 上直接发布 snapshot-bound backend entry points，其他目标继续保留现有 wrapper fallback；fresh `TTestCase_PublicAbi`、fresh external `tests/fafafa.core.simd.publicabi/BuildOrTest.sh test`、fresh `check`、fresh `gate` 全部复验通过，最终 `[GATE] OK`，run-all summary 时间 `2026-03-22 14:07:47` |
| `TrySetActiveBackend(...)` 在 hook-driven postcondition failure 时虽然已经返回 `False`，但旧实现不会回滚先前写下的 `g_BackendForced/g_ForcedBackend`，导致后续 `RegisterBackend(...)` / rebuild 只要重新让 requested backend 变成 dispatchable，就会在没有新 API 调用的情况下把 previously-requested backend 悄悄复活 | 已在 `tests/fafafa.core.simd/fafafa.core.simd.dispatchapi.testcase.pas` 新增 `Test_TrySetActiveBackend_FailedHookMutation_DoesNotLeave_LingeringForcedSelection`，并在 `tests/fafafa.core.simd/fafafa.core.simd.publicabi.testcase.pas` 新增 `Test_PublicApi_FailedHookMutation_DoesNotRevive_PreviouslyRequestedBackend_AfterRestore` 打红；fresh red `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-lingering-force-red-20260322 bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_DispatchAPI,TTestCase_PublicAbi` 直接命中自动选择应回到 backend `<6>` 却被旧 requested backend `<5>` 复活。随后把 `src/fafafa.core.simd.dispatch.pas` 的 `TrySetActiveBackend(...)` 收紧为：若后验最终 active backend 不等于 requested，则在持锁路径内清理 `g_BackendForced` 并把 `g_ForcedBackend` 复位为 `sbScalar`；fresh `TTestCase_DispatchAPI,TTestCase_PublicAbi`、fresh `TTestCase_DirectDispatch,TTestCase_DirectDispatchConcurrent`、fresh `check`、fresh `gate` 全部复验通过，最终 `[GATE] OK`，run-all summary 时间 `2026-03-22 15:32:17` |
| `TrySetActiveBackend(...)` 在 hook-driven postcondition failure 时即使已经回滚了 `g_BackendForced/g_ForcedBackend`，旧实现仍会把 `GetActiveBackend` / `GetSimdPublicApi.ActiveBackendId` 留在 forced path 这次失败尝试临时选出的 `Scalar` fallback，而不是回到 automatic best backend | 已在 `tests/fafafa.core.simd/fafafa.core.simd.dispatchapi.testcase.pas` 新增 `Test_TrySetActiveBackend_FailedHookMutation_Restores_AutomaticBackend`，并在 `tests/fafafa.core.simd/fafafa.core.simd.publicabi.testcase.pas` 新增 `Test_PublicApi_FailedHookMutation_Restores_AutomaticBackend_Immediately` 打红；fresh red `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-failed-hook-auto-restore-red-20260322 bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_DispatchAPI,TTestCase_PublicAbi` 直接命中 automatic backend `<6>` 被 stale scalar `<0>` 覆盖。随后把 `src/fafafa.core.simd.dispatch.pas` 的 `TrySetActiveBackend(...)` 收紧为：postcondition failure 回滚 forced state 后，立即清空初始化状态并重新执行一次 automatic `InitializeDispatch`；同时把旧 `Phase 32/48` 的 “失败后停在 Scalar” 断言收口为 “失败后回到 best dispatchable backend”。fresh `TTestCase_DispatchAPI,TTestCase_PublicAbi`、fresh `TTestCase_DirectDispatch,TTestCase_DirectDispatchConcurrent`、fresh `check`、fresh `gate` 全部复验通过，最终 `[GATE] OK`，run-all summary 时间 `2026-03-22 18:19:32` |
| `TrySetActiveBackend(...)` 在 hook-driven failure rollback 场景下，即使 automatic restore 已经在 return 前重新选回 requested backend，旧实现仍会沿用第一次 forced-attempt 后算出的 `Result=False`，造成“状态已成功、返回值仍失败”的合同漂移 | 已在 `tests/fafafa.core.simd/fafafa.core.simd.dispatchapi.testcase.pas` 新增 `Test_TrySetActiveBackend_RollbackRestore_ReSelects_RequestedBackend_Before_Return`，并在 `tests/fafafa.core.simd/fafafa.core.simd.publicabi.testcase.pas` 新增 `Test_PublicApi_RollbackRestore_ReSelects_RequestedBackend_Before_Return` 打红；fresh red `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-rollback-restore-result-red-20260322 bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_DispatchAPI,TTestCase_PublicAbi` 直接命中 `TrySetActiveBackend should report success when rollback-time restore makes the requested backend active again before return`。随后把 `src/fafafa.core.simd.dispatch.pas` 的 `TrySetActiveBackend(...)` 收紧为：failure rollback 的 `InitializeDispatch` 之后重新读取 published current dispatch，并按 return-time final backend 是否等于 requested backend 重算 `Result`；fresh `TTestCase_DispatchAPI,TTestCase_PublicAbi`、fresh `TTestCase_DirectDispatch,TTestCase_DirectDispatchConcurrent`、fresh `check`、fresh `gate` 全部复验通过，最终 `[GATE] OK`，run-all summary 时间 `2026-03-22 19:44:49` |
| `TrySetActiveBackend(...)` 在 hook-driven failure rollback 场景下，Phase 50 修复后虽然已经会在 automatic restore 重新选回 requested backend 时返回 `True`，但 failure path 更早清掉了 `g_BackendForced/g_ForcedBackend`，导致这次 success 只在 return-time 成立，后续更高优先级 backend 恢复后 active/public ABI 仍会漂回 automatic best backend | 已在 `tests/fafafa.core.simd/fafafa.core.simd.dispatchapi.testcase.pas` 新增 `Test_TrySetActiveBackend_RollbackRestore_Success_Preserves_ForcedSelection`，并在 `tests/fafafa.core.simd/fafafa.core.simd.publicabi.testcase.pas` 新增 `Test_PublicApi_RollbackRestore_Success_Preserves_ForcedSelection` 打红；fresh red `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-force-success-intent-red-20260322 bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_DispatchAPI,TTestCase_PublicAbi` 直接命中 `expected: <1> but was: <6>`。随后把 `src/fafafa.core.simd.dispatch.pas` 的 `TrySetActiveBackend(...)` 收紧为：rollback automatic reinit 后若 return-time final backend 仍等于 requested backend，则同步恢复 `g_ForcedBackend/g_BackendForced`，让 success 语义与持续 forced-selection intent 对齐；fresh `TTestCase_DispatchAPI,TTestCase_PublicAbi`、fresh `check`、fresh `gate` 全部复验通过，最终 `[GATE] OK`，run-all summary 时间 `2026-03-22 20:48:19` |
| `TrySetActiveBackend(...)` 在调用前已经 forced 在 backend A 时，若随后切换到 backend B 并在 hook-driven late failure 中失败，旧实现会把 rollback 一律收口到 automatic best backend，而不是恢复调用前的 forced backend A | 已在 `tests/fafafa.core.simd/fafafa.core.simd.dispatchapi.testcase.pas` 新增 `Test_TrySetActiveBackend_FailedHookMutation_Restores_PreviousForcedBackend`，并在 `tests/fafafa.core.simd/fafafa.core.simd.publicabi.testcase.pas` 新增 `Test_PublicApi_FailedHookMutation_Restores_PreviousForcedBackend` 打红；fresh red `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-previous-forced-rollback-red-20260322 bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_DispatchAPI,TTestCase_PublicAbi` 直接命中 `expected: <5> but was: <6>`。随后把 `src/fafafa.core.simd.dispatch.pas` 的 `TrySetActiveBackend(...)` 收紧为：入口先保存 pre-call `g_BackendForced/g_ForcedBackend`；failure rollback 先保留现有 automatic reinit 的 success-recovery 语义；只有当 return-time final backend 仍不等于 requested 且调用前本来就是 forced mode 时，才恢复 pre-call forced state 并重建 dispatch。fresh `TTestCase_DispatchAPI,TTestCase_PublicAbi`、fresh `check`、fresh `gate` 全部复验通过，最终 `[GATE] OK`，run-all summary 时间 `2026-03-22 22:23:44` |
| `SetActiveBackend(...)` 在 hook-driven late failure 场景下，会把 `TrySetActiveBackend(...)` 已经恢复好的 previous forced backend 又无条件覆盖成 scalar fallback | 已在 `tests/fafafa.core.simd/fafafa.core.simd.dispatchapi.testcase.pas` 新增 `Test_SetActiveBackend_HookLateFailure_Preserves_PreviousForcedBackend`，并在 `tests/fafafa.core.simd/fafafa.core.simd.publicabi.testcase.pas` 新增 `Test_PublicApi_SetActiveBackend_HookLateFailure_Preserves_PreviousForcedBackend` 打红；fresh red `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-setactive-late-failure-red-20260322 bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_DispatchAPI,TTestCase_PublicAbi` 直接命中 `expected: <5> but was: <0>`。随后把 `src/fafafa.core.simd.dispatch.pas` 的 `SetActiveBackend(...)` 收紧为：只对调用开始前就不可选的 requested backend 保留 scalar fallback；若 backend 已进入 selection attempt 但在 hook-driven late failure 中失败，则保留 `TrySetActiveBackend(...)` 已恢复好的 current state。fresh `TTestCase_DispatchAPI,TTestCase_PublicAbi`、fresh `check`、fresh `gate` 全部复验通过，最终 `[GATE] OK`，run-all summary 时间 `2026-03-22 23:38:36` |
| `ResetToAutomaticBackend(...)` 在 dispatch-changed hook 的通知阶段若被一次 late `SetActiveBackend(sbScalar)` 重新 force，旧实现会在 return 后留下 scalar forced fallback，而不是恢复 automatic best backend | 已在 `tests/fafafa.core.simd/fafafa.core.simd.dispatchapi.testcase.pas` 新增 `Test_ResetToAutomaticBackend_HookLateForce_Restores_AutomaticBackend`，并在 `tests/fafafa.core.simd/fafafa.core.simd.publicabi.testcase.pas` 新增 `Test_PublicApi_ResetToAutomaticBackend_HookLateForce_Restores_AutomaticBackend` 打红；fresh red `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-resetauto-lateforce-red-20260323 bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_DispatchAPI,TTestCase_PublicAbi` 直接命中 `expected: <6> but was: <0>`。随后把 `src/fafafa.core.simd.dispatch.pas` 的 `ResetToAutomaticBackend(...)` 收紧为：入口显式清 `g_BackendForced/g_ForcedBackend`，第一次 automatic `InitializeDispatch` 后若发现 hook 又复活 forced state，则在同一持锁路径里再次清 forced 并重建 automatic dispatch；fresh `TTestCase_DispatchAPI,TTestCase_PublicAbi`、fresh `check`、fresh `gate` 全部复验通过，最终 `[GATE] OK`，run-all summary 时间 `2026-03-23 03:46:52` |
| `RegisterBackend(...)` 在 automatic mode 下若在 dispatch-changed hook 的通知阶段被一次 late `SetActiveBackend(sbScalar)` 重新 force，旧实现会在 return 后留下 scalar forced fallback，而不是恢复 automatic best backend | 已在 `tests/fafafa.core.simd/fafafa.core.simd.dispatchapi.testcase.pas` 新增 `Test_RegisterBackend_HookLateForce_Restores_AutomaticBackend`，并在 `tests/fafafa.core.simd/fafafa.core.simd.publicabi.testcase.pas` 新增 `Test_PublicApi_RegisterBackend_HookLateForce_Restores_AutomaticBackend` 打红；fresh red `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-register-lateforce-red-20260323 bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_DispatchAPI,TTestCase_PublicAbi` 直接命中 `expected: <6> but was: <0>`。随后把 `src/fafafa.core.simd.dispatch.pas` 的 `RegisterBackend(...)` 收紧为：入口保存 pre-call `g_BackendForced/g_ForcedBackend`，首次 `InitializeDispatch` 后若发现 hook 改写了 control-plane mode，则恢复 pre-call intent 并再做一次 dispatch 初始化；fresh `TTestCase_DispatchAPI,TTestCase_PublicAbi`、fresh `check`、fresh `gate` 全部复验通过，最终 `[GATE] OK`，run-all summary 时间 `2026-03-23 19:30:01` |
| `TrySetActiveBackend(...)` 在 previous-forced rollback restore 的最后一次 callback 中若被 late `SetActiveBackend(sbScalar)` 再次劫持，旧实现会在 return 后把 previous forced backend 又打回 scalar | 已在 `tests/fafafa.core.simd/fafafa.core.simd.dispatchapi.testcase.pas` 新增 `Test_TrySetActiveBackend_RollbackRestore_LateForce_Preserves_PreviousForcedBackend`，并在 `tests/fafafa.core.simd/fafafa.core.simd.publicabi.testcase.pas` 新增 `Test_PublicApi_RollbackRestore_LateForce_Preserves_PreviousForcedBackend` 打红；fresh red `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-rollback-lateforce-red-20260323 bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_DispatchAPI,TTestCase_PublicAbi` 直接命中 `expected: <5> but was: <0>`。随后把 `src/fafafa.core.simd.dispatch.pas` 的 previous-forced rollback restore 分支收紧为：restore reinit 后若发现 hook 又改写了 `g_BackendForced/g_ForcedBackend`，则恢复 `LPreviousForcedBackend` 并再做一次 dispatch 初始化；fresh `TTestCase_DispatchAPI,TTestCase_PublicAbi`、fresh `check`、fresh `gate` 全部复验通过，最终 `[GATE] OK`，run-all summary 时间 `2026-03-23 20:01:24` |
| `TrySetActiveBackend(...)` 在 automatic caller 的 failure rollback callback 中若被 late `SetActiveBackend(sbScalar)` 再次劫持，旧实现会在 return 后留下 scalar forced fallback，而不是恢复 automatic best backend | 已在 `tests/fafafa.core.simd/fafafa.core.simd.dispatchapi.testcase.pas` 新增 `Test_TrySetActiveBackend_RollbackRestore_LateForce_Restores_AutomaticBackend`，并在 `tests/fafafa.core.simd/fafafa.core.simd.publicabi.testcase.pas` 新增 `Test_PublicApi_RollbackRestore_LateForce_Restores_AutomaticBackend` 打红；fresh red `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-automatic-rollback-lateforce-red-20260323 bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_DispatchAPI,TTestCase_PublicAbi` 直接命中 `expected: <6> but was: <0>`。随后把 `src/fafafa.core.simd.dispatch.pas` 的 automatic rollback 分支收紧为：第一次 rollback `InitializeDispatch` 后若调用前本来是 automatic mode 且发现 hook 又改写了 `g_BackendForced`，则清回 automatic intent 并再做一次 dispatch 初始化；fresh `TTestCase_DispatchAPI,TTestCase_PublicAbi`、fresh `check`、fresh `gate` 全部复验通过，最终 `[GATE] OK`，run-all summary 时间 `2026-03-23 20:36:42` |
| `RegisterBackend(...)` 在调用前已经 forced 在 backend A 时，若 restore callback 中又被 late `ResetToAutomaticBackend(...)` 劫持，旧实现会在 return 后把 previous forced backend 再次漂回 automatic best backend | 已在 `tests/fafafa.core.simd/fafafa.core.simd.dispatchapi.testcase.pas` 新增 `Test_RegisterBackend_HookLateAutomaticReset_Preserves_PreviousForcedBackend`，并在 `tests/fafafa.core.simd/fafafa.core.simd.publicabi.testcase.pas` 新增 `Test_PublicApi_RegisterBackend_HookLateAutomaticReset_Preserves_PreviousForcedBackend` 打红；fresh red `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-register-restore-latereset-red-20260323 bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_DispatchAPI,TTestCase_PublicAbi` 直接命中 `expected: <5> but was: <6>`。随后把 `src/fafafa.core.simd.dispatch.pas` 的 `RegisterBackend(...)` 收紧为：restore reinit 完成后若发现 hook 再次改写了 `g_BackendForced/g_ForcedBackend`，则恢复 `LPreviousBackendForced/LPreviousForcedBackend` 并再做一次 dispatch 初始化；fresh `TTestCase_DispatchAPI,TTestCase_PublicAbi`、fresh `check`、fresh `gate` 全部复验通过，最终 `[GATE] OK`，run-all summary 时间 `2026-03-23 21:22:28` |
| `SetVectorAsmEnabled(...)` 在调用前已经 forced 在 backend A 时，若 restore callback 中又被 late `ResetToAutomaticBackend(...)` 劫持，旧实现会在后续重新启用 vector asm 时把 previous forced backend 再次漂回 automatic best backend | 已在 `tests/fafafa.core.simd/fafafa.core.simd.dispatchapi.testcase.pas` 新增 `Test_SetVectorAsmEnabled_HookLateAutomaticReset_DuringRestore_Preserves_PreviousForcedBackend`，并在 `tests/fafafa.core.simd/fafafa.core.simd.publicabi.testcase.pas` 新增 `Test_PublicApi_SetVectorAsmEnabled_HookLateAutomaticReset_DuringRestore_Preserves_PreviousForcedBackend` 打红；fresh red `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-toggle-restore-latereset-red-20260323 bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_DispatchAPI,TTestCase_PublicAbi` 直接命中 `expected: <1> but was: <6>`。随后把 `src/fafafa.core.simd.dispatch.pas` 的 `SetVectorAsmEnabled(...)` 收紧为：restore reinit 完成后若发现 hook 再次改写了 `g_BackendForced/g_ForcedBackend`，则恢复 `LPreviousBackendForced/LPreviousForcedBackend` 并再做一次 dispatch 初始化；fresh `TTestCase_DispatchAPI,TTestCase_PublicAbi`、fresh `check`、fresh `gate` 全部复验通过，最终 `[GATE] OK`，run-all summary 时间 `2026-03-23 21:49:12` |
| `ResetToAutomaticBackend(...)` 在第二次 automatic restore callback 中若又被 late `SetActiveBackend(sbScalar)` 劫持，旧实现仍会在 return 后留下 scalar forced fallback，而不是恢复 automatic best backend | 已在 `tests/fafafa.core.simd/fafafa.core.simd.dispatchapi.testcase.pas` 新增 `Test_ResetToAutomaticBackend_HookLateForce_DuringRestore_Restores_AutomaticBackend`，并在 `tests/fafafa.core.simd/fafafa.core.simd.publicabi.testcase.pas` 新增 `Test_PublicApi_ResetToAutomaticBackend_HookLateForce_DuringRestore_Restores_AutomaticBackend` 打红；fresh red `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-reset-restore-lateforce-red-20260323 bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_DispatchAPI,TTestCase_PublicAbi` 直接命中 `expected: <6> but was: <0>`。随后把 `src/fafafa.core.simd.dispatch.pas` 的 `ResetToAutomaticBackend(...)` 收紧为：第二次 automatic restore `InitializeDispatch` 后若发现 hook 再次 resurrect 了 `g_BackendForced/g_ForcedBackend`，则再次清回 automatic intent 并再做一次 dispatch 初始化；fresh `TTestCase_DispatchAPI,TTestCase_PublicAbi`、fresh `check`、fresh `gate` 全部复验通过，最终 `[GATE] OK`，run-all summary 时间 `2026-03-23 22:18:51` |
| `TrySetActiveBackend(...)` 在 automatic caller 的第二次 rollback restore callback 中若又被 late `SetActiveBackend(sbScalar)` 劫持，旧实现仍会在 return 后留下 scalar forced fallback，而不是恢复 automatic best backend | 已在 `tests/fafafa.core.simd/fafafa.core.simd.dispatchapi.testcase.pas` 新增 `Test_TrySetActiveBackend_RollbackRestore_LateForce_DuringRestore_Restores_AutomaticBackend`，并在 `tests/fafafa.core.simd/fafafa.core.simd.publicabi.testcase.pas` 新增 `Test_PublicApi_RollbackRestore_LateForce_DuringRestore_Restores_AutomaticBackend` 打红；fresh red `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-automatic-rollback-restore-lateforce-red-20260323 bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_DispatchAPI,TTestCase_PublicAbi` 直接命中 `expected: <6> but was: <0>`。随后把 `src/fafafa.core.simd.dispatch.pas` 的 automatic rollback 分支收紧为：第二次 automatic restore `InitializeDispatch` 后若发现 hook 再次 resurrect 了 `g_BackendForced/g_ForcedBackend`，则再次清回 automatic intent 并再做一次 dispatch 初始化；fresh `TTestCase_DispatchAPI,TTestCase_PublicAbi`、fresh `check`、fresh `gate` 全部复验通过；其中 gate 首次因 `/tmp` tmpfs 满导致外部工具链临时文件写失败，改用 worktree-local `SIMD_OUTPUT_ROOT/TMPDIR` 复跑后最终 `[GATE] OK`，run-all summary 时间 `2026-03-23 23:03:40` |
| `RegisterBackend(...)` / `SetVectorAsmEnabled(...)` 的 previous-forced restore callback 中 late `SetActiveBackend(sbScalar)` 还没有独立 regression guard，导致这两条对称路径是否已被 second-layer closure 守住缺少 fresh 证据 | 已在 `tests/fafafa.core.simd/fafafa.core.simd.dispatchapi.testcase.pas` 新增 `Test_SetVectorAsmEnabled_HookLateForce_DuringRestore_Preserves_PreviousForcedBackend` / `Test_RegisterBackend_HookLateForce_DuringRestore_Preserves_PreviousForcedBackend`，并在 `tests/fafafa.core.simd/fafafa.core.simd.publicabi.testcase.pas` 新增对应 public ABI guards。fresh release `TTestCase_DispatchAPI,TTestCase_PublicAbi`、fresh `check`、fresh `gate` 全绿，证明当前 `src/fafafa.core.simd.dispatch.pas` 的 second-layer closure 已覆盖这两条对称 late-force 路径，本轮无需生产代码修改；run-all summary 时间 `2026-03-24 01:03:16` |
| public ABI backend text getter 在 Phase 19 修完 stale-cache 之后，仍缺少“concurrent `RegisterBackend(...)` churn 下返回的 `PAnsiChar` 只能落在完整 A/B snapshot”这类 machine-readable 并发护栏 | 已在 `tests/fafafa.core.simd/fafafa.core.simd.concurrent.testcase.pas` 新增 `TPublicAbiBackendTextReadWorker` 与 `Test_Concurrent_PublicAbiBackendText_RegisterBackend_ReadConsistency`，使用 1024/2048 字节的 A/B 固定长度 name/description payload、显式 `ThreadSwitch`、以及并发 writer toggle 做压力验证。fresh targeted suite、5 轮重复压测、fresh `check`、fresh `gate` 全绿，说明这轮收口的是 guard gap，而不是 fresh 证实的生产 bug；最终 `[GATE] OK`，run-all summary 时间 `2026-03-24 08:57:37` |
| public ABI cached-table 合同之前只证明“旧 table 还能调用”，但没有 machine-readable 证据去证明“重绑后 fresh getter 返回的是新 table，而旧 cached table 的 metadata 仍保持旧 snapshot” | 已在 `tests/fafafa.core.simd/fafafa.core.simd.publicabi.testcase.pas` 新增 `Test_PublicApi_CachedTable_Preserves_PreviousSnapshot_Metadata_Across_Rebind`，显式断言 rebind 后 `LApiBefore <> LApiAfter`、`LApiBefore.ActiveBackendId/ActiveFlags` 保持旧 snapshot、而 fresh getter 暴露新 metadata。fresh `TTestCase_PublicAbi`、fresh `check`、fresh `gate` 全绿，说明当前 `g_SimdPublicApiStatePtr + g_SimdPublicApiOwnedHead` 的 owned-state 发布模型已经守住这条 consumer-side snapshot 语义；最终 `[GATE] OK`，run-all summary 时间 `2026-03-24 09:14:13` |
| public ABI backend text pointer lifetime 测试之前只做同尺寸字符串分配 churn，没有真实 repeated backend text refresh，因此缺少 “pointer 先被 consumer 观察，再经历多轮 `RegisterBackendText(...)` publication churn 后仍有效” 的 machine-readable 证据 | 已将 `tests/fafafa.core.simd/fafafa.core.simd.publicabi.testcase.pas` 的 `Test_PublicAbi_BackendText_Getters_PreviousPointers_RemainValid_After_Refresh` 升级为真实 repeated `RegisterBackendText(...)` churn：去掉被动 `LNameChurn/LDescriptionChurn` 分配，新增 `BuildChurnNameText/BuildChurnDescriptionText`，每轮都断言 latest getter 看到当前 churn 文本，并每 32 轮回看一次完整 pointer history；同时把 `CHURN_COUNT` 从 `4096` 收到 `1024` 以控制真实 refresh 成本。fresh `TTestCase_PublicAbi`、fresh `check`、fresh `gate` 全绿，说明当前 owned-state publication 在现有宿主机上已经守住这条更强的 pointer-lifetime consumer 合同，本轮收口为 guard gap 而不是生产 bug；最终 `[GATE] OK`，run-all summary 时间 `2026-03-24 10:26:49` |
| `GetBackendOps(backend)` 之前只在 deterministic 场景下覆盖过 unregistered metadata 和 registered empty-text drift，还缺少 “concurrent `RegisterBackend(...)` 切换 enabled/disabled table 时 helper 可见 metadata 与 representative slots 必须来自同一份 snapshot” 的 machine-readable 证据 | 已在 `tests/fafafa.core.simd/fafafa.core.simd.concurrent.testcase.pas` 新增 `TBackendOpsReadWorker` 与 `Test_Concurrent_BackendOps_RegisterBackend_ReadConsistency`，选择当前 non-scalar backend，把 disabled table 的 `Available/Capabilities` 与 `AddF32x4/MulF32x4/AddI32x4/SelectF32x4` 收紧到 scalar-backed 代表态，再让 reader 将 `GetBackendOps(backend)` round-trip 回 dispatch table，只接受完整的 enabled/disabled 两态。fresh `TTestCase_SimdConcurrentFramework`、fresh `check`、fresh `gate` 全绿，说明当前 `TryGetRegisteredBackendDispatchTable(...)` 的 published snapshot 读取和 adapter round-trip 在现有宿主机上没有暴露新的 mixed snapshot；本轮收口为 guard gap 而不是生产 bug；最终 `[GATE] OK`，run-all summary 时间 `2026-03-24 10:43:07` |
| `GetCurrentBackend` 作为 current-active 最薄 façade，之前缺少 “concurrent `RegisterBackend(...)` 把当前 backend 在 enabled/disabled table 间切换时，helper 只能返回完整 active backend 或真实 fallback backend” 的 machine-readable 证据 | 已在 `tests/fafafa.core.simd/fafafa.core.simd.concurrent.testcase.pas` 新增 `TCurrentBackendReadWorker` 与 `Test_Concurrent_CurrentBackend_RegisterBackend_ReadConsistency`，显式把当前 non-scalar backend 重注册成 `Available=False` / `Capabilities=[]` 的 disabled table，再让 reader 仅接受 enabled 原 backend 或 disabled 后的 fallback backend 两种合法结果。fresh `TTestCase_SimdConcurrentFramework`、fresh `check`、fresh `gate` 全绿，说明当前 `GetCurrentBackend -> GetActiveBackend -> current published dispatch snapshot` 读取链在现有宿主机上没有暴露新的 helper-level impossible backend id；本轮收口为 guard gap 而不是生产 bug；最终 `[GATE] OK`，run-all summary 时间 `2026-03-24 10:59:32` |
| x86 backend capability/public ABI 低报 `scMaskedOps`：`SSE2..SSE42/AVX2` 已有 native `Mask*` helper family，`AVX512 vector asm=False` 也会继承 `AVX2` mask helpers，但 capability bits 之前只在 `AVX512 vector asm=True` 时零散宣称 | 已先在 `tests/fafafa.core.simd/fafafa.core.simd.dispatchapi.testcase.pas` 新增 `Test_X86_BackendCapabilities_DoNotUnderclaim_MaskedOps` 与 `Test_X86_BackendCapabilities_Keep_MaskedOps_When_VectorAsmDisabled`，并在 `tests/fafafa.core.simd/fafafa.core.simd.publicabi.testcase.pas` 新增对应 public ABI red；fresh red `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-x86-maskedops-red-20260324 bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_DispatchAPI,TTestCase_PublicAbi` 直接命中 `SSE2` underclaim。随后把 `src/fafafa.core.simd.sse2.pas`、`src/fafafa.core.simd.sse3.register.inc`、`src/fafafa.core.simd.ssse3.register.inc`、`src/fafafa.core.simd.sse41.register.inc`、`src/fafafa.core.simd.sse42.register.inc`、`src/fafafa.core.simd.avx2.register.inc`、`src/fafafa.core.simd.avx512.register.inc` 的 capability set 补入 `scMaskedOps`，并移除旧 `AVX512 scMaskedOps should clear when vector asm is disabled` 断言；fresh targeted suite、fresh `check`、fresh `gate` 全部通过，最终 `[GATE] OK`，run-all summary 时间 `2026-03-24 18:51:26` |
| `RVV asm + OPCODE_READY` 下 `TTestCase_PublicAbi` 之前稳定在 `MemEqual(facade)` AV，看起来像 public ABI / RVV direct-bind 生产缺陷 | 已先用最小诊断确认 failure 时 `CurrentBackend=Scalar` 且 `Dispatch.MemEqual=nil`，随后追到 `Test_PublicApi_RollbackRestore_Success_Preserves_ForcedSelection` 的 early-exit `finally` 把默认 `sbScalar + Default(TSimdDispatchTable)` 回灌进 scalar slot。最小修复是给 public ABI testcase 补 `LTargetTableCaptured` 和入口 global reset，并恢复被误收紧的 `CPURISCV64` direct-bind；fresh RVV `TTestCase_PublicAbi` 与 `TTestCase_DispatchAPI` lanes 全绿，说明这是一条 testcase cleanup bug，不是新的生产 RVV/public-ABI bug |
| RVV native opcode-ready lane 之前默认漏掉 `SIMD_EXPERIMENTAL_RISCVV`，导致 experimental backend 根本没编进 umbrella unit；native lane 一旦真正接通，又立刻暴露 `riscvv.helpers.inc`/`register.inc` 的 asm compile blocker，以及 `scMaskedOps` capability/public ABI underclaim | 已先把 `tests/fafafa.core.simd/docker/run_riscvv_opcode_lane.sh` 的默认 compile/runtime define 集补入 `-dSIMD_EXPERIMENTAL_RISCVV`，再把 `src/fafafa.core.simd.riscvv.pas` 的 `helpers.inc` 收紧为 non-asm only include，并让 `src/fafafa.core.simd.riscvv.register.inc` 在 `RISCVV_ASSEMBLY` 下保留 `DotF32x8/DotF64x2/DotF64x4` 与 `I16x32/I8x64/U8x64/U32x16/U64x8` 那批 fallback-only helper 的 scalar base wiring。native lane compile 真正转绿后，fresh `TTestCase_RISCVVMaskedOpsContract` 直接命中 `scMaskedOps` 与 public ABI `CapabilityBits` 少报；随后把 `src/fafafa.core.simd.riscvv.register.inc` 的 capability set 补入仅在 `LUseVectorAsm=True` 时宣称的 `scMaskedOps`。fresh RVV masked-ops lane 与 fresh release `check` 全绿，说明这轮既修掉了 native evidence harness 假绿，也修掉了一条真实 RVV capability/public-ABI underclaim |
| `TTestCase_NonX86IEEE754` 的 narrow finite RVV path 之前没有显式 `SetVectorAsmEnabled(True)`，导致 native `F64x2` rounding 一直躲在 scalar-backed 伪绿后面；一旦把 runtime 前置条件补真，`RoundF64x2` 第 0 lane 会稳定返回 garbage value | 已在 `tests/fafafa.core.simd/fafafa.core.simd.ieee754.testcase.pas` 新增 `Test_NonX86_NarrowF64x2_RoundTruncFloorCeil_Finite_IfAvailable`，用 fixed finite case 并显式 `SetVectorAsmEnabled(True)`；fresh red `FAFAFA_BUILD_MODE=Release SIMD_RVV_COMPILE_TARGET=project SIMD_RVV_LANE_SUITE=TTestCase_NonX86IEEE754 SIMD_RVV_LANE_SKIP_BENCH=1 SIMD_RVV_RUNTIME_DEFINES='-dSIMD_EXPERIMENTAL_RISCVV -dFAFAFA_SIMD_EXPERIMENTAL_BACKEND_ASM -dFAFAFA_SIMD_ENABLE_RISCVV_ASM -dFAFAFA_SIMD_RISCVV_ASM_COMPILER_READY -dFAFAFA_SIMD_RISCVV_ASM_OPCODE_READY' SIMD_RVV_SUITE_USE_PREBUILT_COMPILER=1 SIMD_RVV_COMPILE_USE_PREBUILT_COMPILER=1 bash tests/fafafa.core.simd/docker/run_riscvv_opcode_lane.sh` 连续 3 次 retry 都命中 `expected: <1> but was: <6.9e-310>`。最小修复是把 `src/fafafa.core.simd.riscvv.register.inc` 中 `RISCVV_ASSEMBLY` 下的 `Ceil/Floor/Round/TruncF64x2` 四个 slot 收回 `Scalar*`；fresh RVV narrow IEEE754 lane、fresh release `check`、fresh serial `gate` 全绿，最终 `[GATE] OK`，run-all summary 时间 `2026-03-25 08:38:12` |
| `RISCVV` narrow `F32x4/F64x2` float core path 仍直接使用 `function(...): TVec*; assembler; nostackframe`，在 `fpc/riscv64` native lane 上会稳定返回 garbage value，简单改 hidden-result 寄存器后又会落成 `Access violation` | 已先在 `tests/fafafa.core.simd/fafafa.core.simd.dispatchapi.testcase.pas` 新增 `Test_NativeNarrowFloatCoreParity_WithVectorAsm_IfAvailable`，显式 `SetVectorAsmEnabled(True)` 并要求 `Add/Abs/Fma/ClampF32x4/F64x2` 既非 scalar 又与 scalar parity；fresh red `FAFAFA_BUILD_MODE=Release SIMD_RVV_COMPILE_TARGET=project SIMD_RVV_LANE_SUITE=TTestCase_NonX86BackendParity SIMD_RVV_LANE_SKIP_BENCH=1 SIMD_RVV_RUNTIME_DEFINES='-dSIMD_EXPERIMENTAL_RISCVV -dFAFAFA_SIMD_EXPERIMENTAL_BACKEND_ASM -dFAFAFA_SIMD_ENABLE_RISCVV_ASM -dFAFAFA_SIMD_RISCVV_ASM_COMPILER_READY -dFAFAFA_SIMD_RISCVV_ASM_OPCODE_READY' SIMD_RVV_SUITE_USE_PREBUILT_COMPILER=1 SIMD_RVV_COMPILE_USE_PREBUILT_COMPILER=1 bash tests/fafafa.core.simd/docker/run_riscvv_opcode_lane.sh` 连续 3 次 retry 都先打到 `AddF32x4` garbage-value red。随后对照 `tests/test_riscvv_wrapper_unit.pas` 的 working sample，确认稳定模式应为 `procedure(...; var r)` + Pascal wrapper，而不是继续依赖 direct vector-return asm。最小生产修复是把 `src/fafafa.core.simd.riscvv.pas` 中代表性的 narrow float vector-return core ops (`Add/Sub/Mul/Div/Abs/Sqrt/Min/Max/Neg/Fma/Clamp` 以及 `Rcp/RsqrtF32x4`) 迁到 wrapper 模式，并把 `FmaF32x4/FmaF64x2` 收敛到与 working sample 一致的 `vfmacc` 写法；fresh RVV `TTestCase_NonX86BackendParity` lane、fresh RVV `TTestCase_NonX86IEEE754,TTestCase_NonX86BackendParity` combined lane、fresh release `check`、fresh serial `gate` 全绿，最终 `[GATE] OK`，run-all summary 时间 `2026-03-25 09:52:25` |
| `RISCVV` narrow `I32x4/I64x2/U32x4/U64x2` integer core path 仍直接使用 direct vector-return asm，在 `fpc/riscv64` native lane 上也会稳定返回 garbage value | 已先在 `tests/fafafa.core.simd/fafafa.core.simd.dispatchapi.testcase.pas` 新增 `Test_NativeNarrowIntegerCoreParity_WithVectorAsm_IfAvailable`，显式 `SetVectorAsmEnabled(True)` 并要求 `Add/And/ShiftLeft/ShiftRightArithI32x4`、`Add/AndI64x2`、`AddU32x4`、`AddU64x2` 既非 scalar 又与 scalar parity；fresh red `FAFAFA_BUILD_MODE=Release SIMD_RVV_COMPILE_TARGET=project SIMD_RVV_LANE_SUITE=TTestCase_NonX86BackendParity SIMD_RVV_LANE_SKIP_BENCH=1 SIMD_RVV_RUNTIME_DEFINES='-dSIMD_EXPERIMENTAL_RISCVV -dFAFAFA_SIMD_EXPERIMENTAL_BACKEND_ASM -dFAFAFA_SIMD_ENABLE_RISCVV_ASM -dFAFAFA_SIMD_RISCVV_ASM_COMPILER_READY -dFAFAFA_SIMD_RISCVV_ASM_OPCODE_READY' SIMD_RVV_SUITE_USE_PREBUILT_COMPILER=1 SIMD_RVV_COMPILE_USE_PREBUILT_COMPILER=1 bash tests/fafafa.core.simd/docker/run_riscvv_opcode_lane.sh` 连续 3 次 retry 都稳定命中 `AddI32x4` lane 0 garbage-value red。随后沿用 Phase 82 已证实可靠的 wrapper 模式，把 `src/fafafa.core.simd.riscvv.pas` 中代表性的 `I32x4/I64x2/U32x4/U64x2` integer core ops 迁到 `procedure(...; var r)` + Pascal wrapper；fresh RVV `TTestCase_NonX86BackendParity` lane、fresh RVV `TTestCase_NonX86IEEE754,TTestCase_NonX86BackendParity` combined lane、fresh release `check`、fresh serial `gate` 全绿，最终 `[GATE] OK`，run-all summary 时间 `2026-03-25 10:40:19` |
| `RISCVV` narrow integer helper `InsertI32x4` 在 Phase 83 的 ABI wrapper closeout 之后，仍会在 native asm lane 上把未目标的尾部 lane 一起写坏；`InsertI64x2` 共享同一实现模式 | 已先在 `tests/fafafa.core.simd/fafafa.core.simd.dispatchapi.testcase.pas` 新增 `Test_NativeNarrowIntegerHelperParity_WithVectorAsm_IfAvailable`，显式 `SetVectorAsmEnabled(True)` 并要求 `InsertI32x4/InsertI64x2` 的 dispatch-table / facade 路径与 scalar parity，同时补 `ExtractI32x4/ExtractI64x2` 基线；fresh red `FAFAFA_BUILD_MODE=Release SIMD_RVV_COMPILE_TARGET=project SIMD_RVV_LANE_SUITE=TTestCase_NonX86BackendParity SIMD_RVV_LANE_SKIP_BENCH=1 SIMD_RVV_RUNTIME_DEFINES='-dSIMD_EXPERIMENTAL_RISCVV -dFAFAFA_SIMD_EXPERIMENTAL_BACKEND_ASM -dFAFAFA_SIMD_ENABLE_RISCVV_ASM -dFAFAFA_SIMD_RISCVV_ASM_COMPILER_READY -dFAFAFA_SIMD_RISCVV_ASM_OPCODE_READY' SIMD_RVV_SUITE_USE_PREBUILT_COMPILER=1 SIMD_RVV_COMPILE_USE_PREBUILT_COMPILER=1 bash tests/fafafa.core.simd/docker/run_riscvv_opcode_lane.sh` 稳定命中 `InsertI32x4 dispatch-table parity lane 3: RISCVV expected: <-404> but was: <0>`。根因不是 ABI，而是 `src/fafafa.core.simd.riscvv.pas` 里的 `vslideup.vx` 从只初始化了 lane0 的临时向量继续拷贝后续 lane，导致插入 `index=2` 时 lane 3 被零覆盖；最小修复是把 `RISCVVInsertI32x4Asm` / `RISCVVInsertI64x2Asm` 改成“整向量复制到结果 + `sw/sd` 单 lane 覆写”，并在 Pascal wrapper 里 clamp `index`。fresh RVV targeted lane、fresh RVV `TTestCase_NonX86IEEE754,TTestCase_NonX86BackendParity` combined lane、fresh release `check`、fresh serial `gate` 全绿，最终 `[GATE] OK`，run-all summary 时间 `2026-03-25 11:38:38` |
| `RISCVV` narrow/wide float insert helper 以及 widened insert family 仍保留 direct-return vector-return asm 时，会在 native asm lane 上稳定触发 `Access violation` | 已先在 `tests/fafafa.core.simd/fafafa.core.simd.dispatchapi.testcase.pas` 新增 `Test_NativeNarrowFloatHelperParity_WithVectorAsm_IfAvailable` 与 `Test_NativeWideInsertHelperParity_WithVectorAsm_IfAvailable`，用 fresh RVV opcode-ready lane 稳定复现 `TTestCase_NonX86BackendParity.Test_NativeNarrowFloatHelperParity_WithVectorAsm_IfAvailable: Access violation` 与 `TTestCase_NonX86BackendParity.Test_NativeWideInsertHelperParity_WithVectorAsm_IfAvailable: Access violation`。根因与 Phase 82/83 相同，仍是 `fpc/riscv64` 下 direct `function(...): TVec*; assembler; nostackframe` 的 vector-return ABI 不稳定；最小修复是把 `RISCVVInsertF32x4`、`RISCVVInsertF64x2` 以及 `RISCVVInsertF32x8/F32x16/F64x4/I32x8/I32x16/I64x4` 统一改成 `procedure(...; var r)` + Pascal wrapper，并在 wrapper 里保留 index clamp。fresh targeted RVV lane 随后转绿，为下一轮继续下钻 helper surface 清出真实 red 面。 |
| `Phase 87` 初版 narrow helper surface parity testcase 先打出的不是生产 red，而是 dispatch contract 自己被 overclaim：`TSimdDispatchTable` 根本没有 `LoadI32x4/SplatI32x4/LoadI64x2/SplatI64x2/SelectI64x2` 这些成员 | 已用 fresh RVV opcode-ready lane 重新复现 compile red，并对照 `src/fafafa.core.simd.dispatch.pas` 与 façade 真相源确认：真实 narrow helper dispatch contract 只有 `Load/Splat/Select/ExtractF32x4`、`Load/Splat/Select/ExtractF64x2`、`Select/ExtractI32x4`、`ExtractI64x2`。在把 testcase 收缩到真实 contract 后，同一条 fresh lane 立刻转成新的 production red：`TTestCase_NonX86BackendParity.Test_NativeNarrowHelperSurfaceParity_WithVectorAsm_IfAvailable: Access violation`。根因是剩余的 `RISCVVLoadF32x4`、`RISCVVSplatF32x4`、`RISCVVSelectF32x4`、`RISCVVLoadF64x2`、`RISCVVSplatF64x2`、`RISCVVSelectF64x2` 仍保留 direct-return asm；最小修复是把这 6 个 helper 也迁到 `procedure(...; var r)` + wrapper。fresh RVV targeted lane、fresh combined `TTestCase_NonX86IEEE754,TTestCase_NonX86BackendParity` lane、fresh release `check`、fresh serial `gate` 全绿，最终 `[GATE] OK`，run-all summary 时间 `2026-03-25 12:53:56`。 |
| `RVV` aligned-load / zero helper 在 narrow helper surface 收口之后仍可继续命中 native asm `Access violation`，说明 direct-return ABI 风险还残留在更小的 helper 上 | 已在 `tests/fafafa.core.simd/fafafa.core.simd.dispatchapi.testcase.pas` 新增 `Test_NativeAlignedLoadAndZeroParity_WithVectorAsm_IfAvailable`，显式 `SetVectorAsmEnabled(True)` 后要求 `LoadF32x4Aligned`、`ZeroF32x4`、`ZeroF64x2` 的 dispatch-table / facade 路径都与 scalar parity；fresh red 仍稳定命中 `TTestCase_NonX86BackendParity.Test_NativeAlignedLoadAndZeroParity_WithVectorAsm_IfAvailable: Access violation`。最小修复是把 `src/fafafa.core.simd.riscvv.pas` 中 `RISCVVLoadF32x4Aligned`、`RISCVVZeroF32x4`、`RISCVVZeroF64x2` 统一改成 `procedure(...; var r)` + Pascal wrapper。fresh RVV targeted lane、fresh release `check`、fresh serial `gate` 随后全绿，最终 `[GATE] OK`，run-all summary 时间 `2026-03-25 19:53:28`。 |

## Latest Findings (Phase 92)
- `Phase 92` 继续把 RVV native evidence 推进到公开 F64 vector math contract，并新增 `TTestCase_NonX86BackendParity.Test_NativeF64DotParity_WithVectorAsm_IfAvailable`，把 `DotF64x2`、`DotF64x4` 纳入 fresh parity 面。
- 第 1 层 fresh red 先暴露 wiring 空洞，而不是算法误差：
  - `DotF64x2 unexpectedly falls back to scalar slot: RISCVV`
  - 这说明 `src/fafafa.core.simd.riscvv.register.inc` 在 `RISCVV_ASSEMBLY` 下之前根本没有把 `DotF64x2/DotF64x4` 接进 RVV backend，公开 dispatch contract 在 asm build 里一直静默沿用 scalar base slot。
- 把 slot 接入后，fresh lane 并没有直接转绿，而是继续暴露 runtime 稳定性问题：
  - `TTestCase_NonX86BackendParity.Test_NativeF64DotParity_WithVectorAsm_IfAvailable: Access violation`
  - 这说明这两个入口在当前阶段不适合继续走那版直写 RVV reduce asm。
- 最小修复已经完成：
...
- fresh RVV targeted lane、fresh release `check`、fresh serial `gate` 已全部重新转绿：
  - targeted lane summary：`tests/fafafa.core.simd/logs/rvv-opcode-lane-20260326-085033/summary.md`
  - `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-check-20260326-phase92 bash tests/fafafa.core.simd/BuildOrTest.sh check`：PASS
  - `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-gate-20260326-phase92-serial bash tests/fafafa.core.simd/BuildOrTest.sh gate`：PASS，run-all summary 时间 `2026-03-26 08:55:03`
- 当前最值得继续追的方向进一步收窄为：
  - vector math edge contract，优先看 zero / tiny normalize 是否还有语义漂移
  - 然后回到 x86 / non-x86 `SetVectorAsmEnabled(True -> False)` rebuild 路径里的 stale dispatch / stale capability 合同

## Latest Findings (Phase 93)
- `Phase 93` 继续把 RVV native evidence 推进到 vector math edge contract，并新增 `TTestCase_NonX86BackendParity.Test_NativeNormalizeEdgeParity_WithVectorAsm_IfAvailable`，把 `NormalizeF32x3/NormalizeF32x4` 的 zero / tiny 输入纳入 fresh parity 面。
- fresh red 不是算法数值误差，而是非常明确的 tail-lane contract hole：
  - `NormalizeF32x3 zero dispatch-table parity lane 3: RISCVV expected: <0> but was: <4.55e-41>`
  - 同一条 lane 在脚本自带 3 次 retry 中稳定重现，只是脏值不同，说明 `lane 3` 在返回时未被定义。
- 根因已经确认：
  - `ScalarNormalizeF32x3` 的稳定语义一直是只归一化前 3 个分量，并强制 `w=0`
  - `src/fafafa.core.simd.riscvv.pas` 里的 `RISCVVNormalizeF32x3Asm` 在 zero-length 分支下仍保持 `vl=3`，只写回 3 个 lane
  - 因此 `RISCVVNormalizeF32x3` wrapper 返回时，`Result.f[3]` 会保留未初始化脏值；这解释了为什么 tiny / non-zero path 之前没红，但 zero path 一补 edge test 就稳定暴露
- 最小修复已经完成：
  - `src/fafafa.core.simd.riscvv.pas` 中 `RISCVVNormalizeF32x3` 现在会先把四个 lane 全部清零，再调用 asm，并在返回后再次显式写 `Result.f[3] := 0.0`
  - 这把 `NormalizeF32x3` 的 tail-lane 合同固定在 Pascal wrapper 层，避免继续依赖 RVV zero-branch 的部分写回行为
- fresh RVV targeted lane、fresh release `check`、fresh serial `gate` 已全部重新转绿：
  - targeted lane red summary：`tests/fafafa.core.simd/logs/rvv-opcode-lane-20260326-095445/summary.md`
  - targeted lane green summary：`tests/fafafa.core.simd/logs/rvv-opcode-lane-20260326-100140/summary.md`
  - `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-check-20260326-phase93 bash tests/fafafa.core.simd/BuildOrTest.sh check`：PASS
  - `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-gate-20260326-phase93-serial bash tests/fafafa.core.simd/BuildOrTest.sh gate`：PASS，run-all summary 时间 `2026-03-26 10:06:28`
- 当前最值得继续追的方向已经从 normalize edge 转回 control-plane：
  - x86 / non-x86 `SetVectorAsmEnabled(True -> False)` rebuild 路径是否还存在 stale dispatch / stale capability drift
  - 若再继续深审 RVV vector math，更适合转向尚未专门建 guard 的 `F64` 其他 vector-math 入口，而不是重复围绕 `NormalizeF32x3` 展开

## Latest Findings (Phase 94)
- `Phase 94` 先没有继续碰生产实现，而是把 `SetVectorAsmEnabled(False -> True)` 的 automatic-mode late-force 合同补成了 machine-readable guard：
  - `tests/fafafa.core.simd/fafafa.core.simd.dispatchapi.testcase.pas` 新增：
  ...
  - `tests/fafafa.core.simd/fafafa.core.simd.publicabi.testcase.pas` 新增对应 public ABI 版本
- 这批 testcase 用现成 synthetic hook 明确锁定：
  - automatic caller 先在 `vector asm=False` 稳定落回 automatic best backend
  - 然后在重新 `SetVectorAsmEnabled(True)` 的通知阶段，hook 即使 late `SetActiveBackend(sbScalar)`，return-time current/public ABI 仍必须回到 automatic best backend，而不能卡在 stale scalar fallback
- fresh 结果没有打出新的生产红：
  - 本地主机：
  ...
  - native RVV lane：
    ...
    - PASS，summary：`tests/fafafa.core.simd/logs/rvv-opcode-lane-20260326-103401/summary.md`
  - fresh release `check`：
    ...
    - PASS
- 这轮结论是 coverage closeout，而不是 bug closeout：
  - 当前 `SetVectorAsmEnabled` automatic-mode re-enable 路径，已经在 local + RVV native 两侧被证明能挡住 late scalar force
  - 因此继续在这条 automatic-mode toggle 合同上深挖的 ROI 已经下降；下一轮更适合转回新的 fresh native red，优先看 `F64` vector-math 其他入口

## Latest Findings (Phase 95)
- 这轮先对 `F64` vector-math 的后续候选做了一个 fresh 证据分流：
  ...
  - fresh RVV opcode lane 结果保持 PASS，说明“`ReduceMax` 返回值污染 `f10`，进而拖偏后续 `ReduceAddF64x4/F64x8`”这条假设在当前 lane 上没有 fresh red
  - 这次 green 很重要：它直接否掉了继续改 `riscvv` reduction seed 的冲动，避免又回到已经证伪的分支上
- 真正打出来的 fresh red 其实落在 stable-boundary façade/current-dispatch contract，而不是 native RVV 实现：
  ...
  - synthetic re-register 会把当前 active backend 的 `ReduceAdd/Min/Max/MulF64x4` slot 全部替换成 sentinel 返回值，再断言 façade 必须跟 current dispatch table 对齐
  - fresh red 命中：
    - `VecF64x4ReduceAdd should track current dispatch table after re-register expected: <401.25> but was: <-2>`
- 根因已经确认：
  - `GetDispatchTable^.ReduceAddF64x4/ReduceMinF64x4/ReduceMaxF64x4/ReduceMulF64x4` 在 synthetic re-register 后确实已经切到新 slot
  - 但 `src/fafafa.core.simd.pas` 中 `VecF64x4ReduceAdd/ReduceMin/ReduceMax/ReduceMul` 仍直接执行手写 scalar reduction，没有走 current dispatch snapshot
  - 这说明当前 bug 不是 backend slot 缺失，也不是 RVV runtime 噪音，而是 façade 与 in-repo dispatch contract 漂移
- 最小修复已经完成：
  ...
  - 上述 4 个 façade 现在统一采用 dispatch-first + scalar fallback，与 current backend re-register 后的 dispatch snapshot 保持一致
- fresh release 复验已经闭环：
  - `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-f64x4-reduce-facade-green-20260326 bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_DispatchAPI`：PASS
  - `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-f64x4-reduce-gate-20260326 bash tests/fafafa.core.simd/BuildOrTest.sh gate`：PASS，最终 `[GATE] OK`
  - `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-f64x4-reduce-check-serial-20260326 bash tests/fafafa.core.simd/BuildOrTest.sh check`：PASS
- 过程里额外确认了一条操作约束：
  - 在同一 worktree 上并行跑 `check` 与 `gate` 会互踩共享的 `bin2/lib2`
  - 本轮第一次 `check` 的 `nonx86 opt-in neon` 假失败就是这个原因；改成串行后立即恢复全绿，说明不是新回归

## Latest Findings (Phase 96)
- `VecF32x8Reduce*` 已经被 fresh evidence 证明与 `TSimdDispatchTable` 合同漂移，而且是 `VecF64x4Reduce*` 同类问题，不是新的 backend/native lane 噪音：
  - `tests/fafafa.core.simd/fafafa.core.simd.dispatchapi.testcase.pas` 新增 `TTestCase_DispatchAPI.Test_VecF32x8ReduceFacade_Tracks_CurrentDispatchTable_After_ReRegister`
  - synthetic re-register 会把当前 active backend 的 `ReduceAdd/Min/Max/MulF32x8` slot 全部替换成 sentinel 返回值，再断言 façade 必须跟 current dispatch table 对齐
  - fresh red 结果：
    - `FAFAFA_BUILD_MODE=Release bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_DispatchAPI`
    - 失败点：
      - `VecF32x8ReduceAdd should track current dispatch table after re-register expected: <123.5> but was: <-3.25>`
- 根因已经确认：
  - `src/fafafa.core.simd.dispatch.pas` 里本来就有 `ReduceAdd/Min/Max/MulF32x8` slot，说明不是“这个向量族根本没 dispatch contract”
  - synthetic re-register 后，`GetDispatchTable^.Reduce*F32x8` 已经切到新 slot
  - 但 `src/fafafa.core.simd.pas` 里的 `VecF32x8ReduceAdd/ReduceMin/ReduceMax/ReduceMul` 仍直接执行手写 scalar reduction，所以 façade 返回值与 current dispatch snapshot 分叉
- 这条 red 的价值在于它把“intentional scalar facade 设计”与“遗漏接线”分开了：
  - `F64x8/F32x16` 这类 sibling façade 已经是 dispatch-first
  - `F32x8` 同时又拥有 dispatch slot
  - 因此当前更合理的解释是历史遗漏，而不是刻意保留的 stable boundary
- 最小修复已经完成：
  - `src/fafafa.core.simd.pas` 中 `VecF32x8ReduceAdd`
  - `src/fafafa.core.simd.pas` 中 `VecF32x8ReduceMin`
  - `src/fafafa.core.simd.pas` 中 `VecF32x8ReduceMax`
  - `src/fafafa.core.simd.pas` 中 `VecF32x8ReduceMul`
  - 这 4 个 façade 现在统一采用 dispatch-first + scalar fallback，与 `VecF64x4Reduce*` 和 wide sibling façade 保持一致
- fresh release 复验已经闭环：
  ...
  - `TMPDIR=/home/dtamade/projects/fafafa.core/.claude/worktrees/simd-external-evidence/.simd-output/tmp FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/home/dtamade/projects/fafafa.core/.claude/worktrees/simd-external-evidence/.simd-output/phase96-check bash tests/fafafa.core.simd/BuildOrTest.sh check`：PASS
  - `TMPDIR=/home/dtamade/projects/fafafa.core/.claude/worktrees/simd-external-evidence/.simd-output/tmp FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/home/dtamade/projects/fafafa.core/.claude/worktrees/simd-external-evidence/.simd-output/phase96-gate bash tests/fafafa.core.simd/BuildOrTest.sh gate`：PASS，最终 `[GATE] OK`
- roadmap 真相源也做了最小同步：
  - `backlog.md` 里把已经闭环的 `SIMD-B07` 从 active queue 移出，避免继续围着旧 blocker 排期
  - 同时新增 `SIMD-B21(candidate)`，把当前真正还在收口的主线明确成 reduction façade / current-dispatch contract sweep

## Latest Findings (Phase 97)
- `VecF64x2Reduce*` 也被 fresh evidence 证明与 `TSimdDispatchTable` 合同漂移，而且模式与 `VecF64x4Reduce*` / `VecF32x8Reduce*` 完全一致：
  - `tests/fafafa.core.simd/fafafa.core.simd.dispatchapi.testcase.pas` 新增 `TTestCase_DispatchAPI.Test_VecF64x2ReduceFacade_Tracks_CurrentDispatchTable_After_ReRegister`
  - synthetic re-register 会把当前 active backend 的 `ReduceAdd/Min/Max/MulF64x2` slot 全部替换成 sentinel 返回值，再断言 façade 必须跟 current dispatch table 对齐
  - fresh red 结果：
    - `FAFAFA_BUILD_MODE=Release bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_DispatchAPI`
    - 失败点：
      - `VecF64x2ReduceAdd should track current dispatch table after re-register expected: <77.5> but was: <5.25>`
- 根因已经确认：
  - `src/fafafa.core.simd.dispatch.pas` 里本来就有 `ReduceAdd/Min/Max/MulF64x2` slot，说明不是“这个向量族没有 dispatch contract”
  - synthetic re-register 后，`GetDispatchTable^.Reduce*F64x2` 已经切到新 slot
  - 但 `src/fafafa.core.simd.pas` 里的 `VecF64x2ReduceAdd/ReduceMin/ReduceMax/ReduceMul` 仍直接执行手写 scalar reduction，所以 façade 返回值与 current dispatch snapshot 分叉
- 最小修复已经完成：
  - `src/fafafa.core.simd.pas` 中 `VecF64x2ReduceAdd`
  - `src/fafafa.core.simd.pas` 中 `VecF64x2ReduceMin`
  - `src/fafafa.core.simd.pas` 中 `VecF64x2ReduceMax`
  - `src/fafafa.core.simd.pas` 中 `VecF64x2ReduceMul`
  - 这 4 个 façade 现在统一采用 dispatch-first + scalar fallback，与 `VecF64x4Reduce*`、`VecF32x8Reduce*` 以及已有 wide sibling façade 保持一致
- fresh release 复验已经闭环：
  - `FAFAFA_BUILD_MODE=Release bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_DispatchAPI`：PASS
  - `TMPDIR=/tmp/simd-f64x2-reduce-directdispatch-20260327 FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-f64x2-reduce-directdispatch-20260327 bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_DirectDispatch`：PASS
  - `TMPDIR=/tmp/simd-f64x2-reduce-check-20260327 FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-f64x2-reduce-check-20260327 bash tests/fafafa.core.simd/BuildOrTest.sh check`：PASS
  - `TMPDIR=/tmp/simd-f64x2-reduce-gate-20260327 FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-f64x2-reduce-gate-20260327 bash tests/fafafa.core.simd/BuildOrTest.sh gate`：PASS，最终 `[GATE] OK`
- roadmap 真相源最小同步：
  - `backlog.md` 中 `SIMD-B21(candidate)` 的已收口范围补入 `VecF64x2Reduce*`
  - 目前 reduction façade / current-dispatch contract sweep 的剩余关注点，主要是继续确认是否还存在其他 sibling 历史遗漏

## Latest Findings (Phase 98)
- 这轮没有新增 Pascal 生产实现 bug；fresh evidence 指向的是 3 条自动化/治理层面的真实缺口：
  - `tests/fafafa.core.simd/docker/run_riscvv_opcode_lane.sh` 的默认 define 契约漂移：
    - compile/lane 默认已经带 `-dFAFAFA_SIMD_RISCVV_ASM_OPCODE_READY`
    - runtime 默认却少了 opcode-ready，而且也没显式带 `-dSIMD_EXPERIMENTAL_RISCVV`
    - 结果是脚本默认值本身在制造“compile 能看到的 RVV asm 合同”和“runtime 实际跑到的合同”之间的假分叉
  - `tests/fafafa.core.simd/nonx86.optin/` 作为 fresh `check/gate` 的隔离输出子树，之前没有仓库级 ignore，容易把本地运行产物重新带回 worktree
  - `docs/plans/2026-03-24-simd-audit-closeout-roadmap.md` 仍然像当前 active roadmap，但实际 active queue 已在 `2026-03-27` 前切到 `backlog.md` 里的 `SIMD-B21(candidate)`
- 最小修复已经完成：
  - `run_riscvv_opcode_lane.sh` 现在把 `RUNTIME_DEFINES` 默认收口到与 `LANE_DEFINES` 相同的 opcode-ready 合同；若确实需要“编译强、运行弱”的 lane，只能显式 override `SIMD_RVV_RUNTIME_DEFINES`
  - `.gitignore` 已新增 `tests/fafafa.core.simd/nonx86.optin/`
  - `2026-03-24` closeout roadmap 顶部已明确标记为 completed historical batch，并指向当前 truth sources
- 新鲜复验证据：
  - `bash -n tests/fafafa.core.simd/docker/run_riscvv_opcode_lane.sh`：PASS
  - `git check-ignore -v tests/fafafa.core.simd/nonx86.optin/riscvv/logs/test.txt`：PASS
  - `TMPDIR=/tmp/simd-check-20260327-phase98-tmp FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-check-20260327-phase98 bash tests/fafafa.core.simd/BuildOrTest.sh check`：PASS
    - 关键观测：
      - 主 `CHECK` 链全绿
      - `NONX86-OPTIN neon/riscvv` 两条 fresh compile smoke 都在隔离目录下完成并通过
  - `TMPDIR=/tmp/simd-gate-20260327-phase98-tmp FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-gate-20260327-phase98 bash tests/fafafa.core.simd/BuildOrTest.sh gate`：PASS
    - 关键观测：
      - fast-gate `1/6..6/6` 全部通过，最终 `[GATE] OK`
      - Windows evidence verify 仍是默认 `SKIP optional`，说明本轮没有重新把已完成的 B07 closeout 错误拉回 active blocker
- 路线图评审结论也更明确了：
  - `2026-03-24` 文档现在应该被当作已完成 closeout 批次的审计记录，而不是当前优先级来源
  - 当前 SIMD 真实 active 话题仍是 `SIMD-B21(candidate)`，即 reduction façade / current-dispatch contract sweep
  - 因此后续建议不应再把“补 Windows B07 证据”或“重复 closeout Batch 69/70”当成主线 blocker

## Latest Findings (Phase 99)
- 这轮继续按 `SIMD-B21(candidate)` 做 sibling reduction sweep，但没有再打出新的 production façade drift：
  - 静态检查先确认剩余还没 machine-readable 守住的 wide sibling 主要是 `VecF64x8Reduce*` 与 `VecF32x16Reduce*`
  - `src/fafafa.core.simd.pas` 中这两组 façade 本身已经是 direct dispatch 形式，不像前面 `VecF64x4/F32x8/F64x2` 那样仍有手写 scalar reduction 漂移
- 为了把“看起来没问题”升级成可回归的 contract，本轮新增了两条 fresh re-register guard：
  ...
  - 同时配套新增对应的 synthetic `ReduceAdd/Min/Max/Mul` sentinel helpers
- fresh release 结果：
  - `TMPDIR=/tmp/simd-b21-sibling-dispatchapi-20260327-tmp FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-b21-sibling-dispatchapi-20260327 bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_DispatchAPI`：PASS
  - `TMPDIR=/tmp/simd-b21-sibling-check-20260327-tmp FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-b21-sibling-check-20260327 bash tests/fafafa.core.simd/BuildOrTest.sh check`：PASS
- 这轮结论：
  - `VecF64x8Reduce*` 与 `VecF32x16Reduce*` 当前已经跟随 current dispatch table，不需要新的生产修复
  - `SIMD-B21(candidate)` 的剩余范围因此进一步收窄，当前更像“补齐最后的 sibling 守卫与确认无遗漏”，而不是继续预期会有同类 façade bug 成批出现

## Latest Findings (Phase 100)
- `VecF32x4Reduce*` 这轮也没有暴露新问题，而且进一步确认它其实不是 B21 的漏网 sibling：
  - `tests/fafafa.core.simd/fafafa.core.simd.dispatchapi.testcase.pas` 里本来就已经有
    `TTestCase_DispatchAPI.Test_VecF32x4ReduceFacade_Tracks_CurrentDispatchTable_After_ReRegister`
  - 对应的 `SyntheticReduce*F32x4CurrentDispatch` helpers 也已经在位
- 这轮的实际收获不是新增测试，而是排除了“F32x4 还没被 machine-readable guard 守住”这个误判：
  - 定向 `DispatchAPI` release 复验：PASS
  - fresh release `check`：PASS
- 因此现在公开 float reduction façade 可以整体看作已经收口：
  - 前三组真实漂移已修：`VecF64x4Reduce*`、`VecF32x8Reduce*`、`VecF64x2Reduce*`
  - 其余三组 guard 已在位并 fresh 绿：`VecF32x4Reduce*`、`VecF64x8Reduce*`、`VecF32x16Reduce*`
- 路线图结论：
  - `SIMD-B21(candidate)` 现在可以视为 completed，而不应继续保留在 active queue
  - 后续 SIMD 工作应重新选题，不再把 reduction façade sibling sweep 当作当前主线

## Latest Findings (Phase 101)
- `SIMD-B21(candidate)` 现在可以正式关闭：
  - `src/fafafa.core.simd.pas` 里的公开 `Reduce*` façade 只存在于 float 家族，没有额外的公开 non-float sibling 需要继续按这条路线收口
  - 这 6 组 float façade 当前状态已经完整明确：
    - 已修真实漂移：`VecF64x4Reduce*`、`VecF32x8Reduce*`、`VecF64x2Reduce*`
    - 已有 guard 并 fresh 绿：`VecF32x4Reduce*`、`VecF64x8Reduce*`、`VecF32x16Reduce*`
- final closeout evidence：
  - `TMPDIR=/tmp/simd-b21-closeout-gate-20260327-tmp FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-b21-closeout-gate-20260327 bash tests/fafafa.core.simd/BuildOrTest.sh gate`：PASS
  - 关键观测：
    - fast-gate 全链路通过，最终 `[GATE] OK`
    - `DispatchAPI` / `DirectDispatch` / `DirectDispatchConcurrent` 全绿
    - public ABI smoke、nonx86 opt-in smoke 全绿
    - Windows evidence verify 仍为默认 `SKIP optional`，不影响本轮 B21 closeout

## Latest Findings (Phase 103)
- 新一轮候选已经确认是一个真实的 façade/current-dispatch drift：
  - `src/fafafa.core.simd.dispatch.pas` 明确提供 `AbsF64x2`、`SqrtF64x2`、`MinF64x2`、`MaxF64x2`
  - 但 `src/fafafa.core.simd.pas` 修复前的 `VecF64x2Abs/Sqrt/Min/Max` 仍直接执行本地标量实现
  - 所以 synthetic re-register 后，dispatch table 已切到新 slot，而 façade 仍停在旧结果
- fresh red 证据：
  - `TMPDIR=/tmp/simd-b22-f64x2math-red-20260327-tmp FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-b22-f64x2math-red-20260327 bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_DispatchAPI`：FAIL
  - 失败点：
...
- 最小修复已经完成：
  - `src/fafafa.core.simd.pas`
    ...
    - `VecF64x2Min`
    - `VecF64x2Max`
  - 这 4 个 façade 现在统一采用 dispatch-first + scalar fallback
- 本轮还顺手收掉了一个测试治理缺口：
  - `tests/fafafa.core.simd/fafafa.core.simd.dispatchapi.testcase.pas` 原来已有两条 method declaration 但没有实现：
  ...
  - 它们在某些 build 路径里会把 `gate` 打成 forward declaration error
  - 本轮已补齐真实 guard；中途误补出重复实现，随后已按 build log 诊断并移除重复体，最终 `DispatchAPI` / `gate` 恢复全绿
- fresh release 复验：
  - `TMPDIR=/tmp/simd-b22-f64x2math-green-20260327-tmp FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-b22-f64x2math-green-20260327 bash tests/fafafa.core.simd/BuildOrTest.sh test --suite=TTestCase_DispatchAPI`：PASS，`[TEST] OK`、`[LEAK] OK`
  - `TMPDIR=/tmp/simd-b22-f64x2math-check-20260327-tmp FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-b22-f64x2math-check-20260327 bash tests/fafafa.core.simd/BuildOrTest.sh check`：PASS
  - `TMPDIR=/tmp/simd-b22-f64x2math-gate2-20260327-tmp FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-b22-f64x2math-gate2-20260327 bash tests/fafafa.core.simd/BuildOrTest.sh gate`：PASS，最终 `[GATE] OK`

## Latest Findings (Phase 104)
- `Phase 98` 收掉的是 RVV opcode lane 的默认 define 漂移本身；这轮继续确认后发现，真正还没守住的是“修复后的合同没有 machine-readable guard”：
  - `tests/fafafa.core.simd/docker/run_riscvv_opcode_lane.sh` 现态已经把
    `SIMD_EXPERIMENTAL_RISCVV`、`FAFAFA_SIMD_RISCVV_ASM_OPCODE_READY`
    一起收进默认 `LANE_DEFINES`
  - `COMPILE_DEFINES` 与 `RUNTIME_DEFINES` 也都默认从 `LANE_DEFINES` 派生
  - 但此前默认 `check/gate` 并不会验证这些关键片段是否仍然存在，后续维护时很容易再次滑回“compile/runtime 口径分叉”
- 最小修复已经完成：
  - 在 `tests/fafafa.core.simd/BuildOrTest.sh` 新增 `check_riscvv_opcode_lane_contract_guard()`
  - 该 guard 静态校验：
    - `RISCV_EXPERIMENTAL_DEFINE` 默认存在且指向 `-dSIMD_EXPERIMENTAL_RISCVV`
    - `RISCV_OPCODE_DEFINE` 默认存在且指向 `-dFAFAFA_SIMD_RISCVV_ASM_OPCODE_READY`
    - `LANE_DEFINES` 默认同时包含实验开关、backend/compiler/opcode-ready 三层定义
    - `COMPILE_DEFINES` / `RUNTIME_DEFINES` 默认都由 `LANE_DEFINES` 派生
    - suite / bench runtime 命令仍显式消费 `${RUNTIME_DEFINES}`
  - 新 guard 已接入默认 `gate_step_build_check()` 与 `check` action 主链
- 中途出现过一条 guard 自身的 shell 细节错误：
  - 首次 fresh `check` 在 `set -u` 下报 `RUNTIME_DEFINES: unbound variable`
  - 根因是静态 pattern 字符串被 shell 提前展开
  - 随后已改成字面量匹配，再次复验通过
- fresh release 复验证据：
  - `bash -n tests/fafafa.core.simd/BuildOrTest.sh`：PASS
  - `bash -n tests/fafafa.core.simd/docker/run_riscvv_opcode_lane.sh`：PASS
  - `TMPDIR=/tmp/simd-check-20260327-phase104b-tmp FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/tmp/simd-check-20260327-phase104b bash tests/fafafa.core.simd/BuildOrTest.sh check`：PASS
    - 关键观测：
      - `[CHECK] OK (RVV opcode lane compile/runtime contract guard present)`
      - `NONX86-OPTIN neon/riscvv` compile smoke 仍保持 PASS
- 这轮结论：
  - RVV opcode lane 现在不只是“脚本当前写对了”，而是“默认 release 主链会持续盯住它是否还写对”
  - 后续如果有人误删 `SIMD_EXPERIMENTAL_RISCVV`、把 `RUNTIME_DEFINES` 改回弱于 `COMPILE_DEFINES`，或者让 runtime 命令不再消费 `${RUNTIME_DEFINES}`，默认 `check` 就会立即打红

## Latest Findings (Phase 105)
- 这轮 fresh 审查打出的不是新的 `riscvv` Pascal 生产实现 bug，而是一条脚本级默认工具链漂移：
  - `tests/fafafa.core.simd/docker/run_riscvv_opcode_lane.sh` 默认 `SIMD_RVV_USE_PREBUILT_COMPILER=1`
  - `COMPILE_USE_PREBUILT_COMPILER` 会继承该默认值
  - 但修复前的 `SUITE_USE_PREBUILT_COMPILER` / `BENCH_USE_PREBUILT_COMPILER` 仍默认 `0`
  - 结果是 compile-only 用带 RVV opcode patch 的 prebuilt compiler，suite/bench 却回退到容器内默认 `ppcrv64`
- fresh red 证据：
  - `TMPDIR=/home/dtamade/projects/fafafa.core/.claude/worktrees/simd-external-evidence/.simd-output/tmp FAFAFA_BUILD_MODE=Release SIMD_RVV_LANE_SKIP_BENCH=1 bash tests/fafafa.core.simd/docker/run_riscvv_opcode_lane.sh`：FAIL
  - 关键失败点：
    - suite 阶段编译 `fafafa.core.simd.test.lpr` 时大量报 `Error: Unrecognized opcode vsetivli`
    - 同类错误覆盖 `vle32.v` / `vfadd.vv` / `vse32.v` 等 RVV opcode
  - 结论：这是 lane 自己制造的 false failure，不是因为当前 `src/fafafa.core.simd.riscvv.pas` 再次回退成“不支持 RVV opcode”
- TDD / regression guard：
  - 先在 `tests/fafafa.core.simd/BuildOrTest.sh` 的 `check_riscvv_opcode_lane_contract_guard()` 里追加两条 machine-readable pattern：
  ...
  - fresh `check` 立即按预期打红，证明旧脚本确实还没守住这条默认关系
- 最小修复已经完成：
  - `tests/fafafa.core.simd/docker/run_riscvv_opcode_lane.sh`
    ...
    - 新增注释说明：需要降级到非 prebuilt toolchain 时，必须显式设置 `SIMD_RVV_*_USE_PREBUILT_COMPILER=0`
- fresh release 复验证据：
  - `TMPDIR=/home/dtamade/projects/fafafa.core/.claude/worktrees/simd-external-evidence/.simd-output/tmp FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/home/dtamade/projects/fafafa.core/.claude/worktrees/simd-external-evidence/.simd-output/green-rvv-prebuilt-guard-20260327 bash tests/fafafa.core.simd/BuildOrTest.sh check`：PASS
  - `TMPDIR=/home/dtamade/projects/fafafa.core/.claude/worktrees/simd-external-evidence/.simd-output/tmp FAFAFA_BUILD_MODE=Release SIMD_RVV_LANE_SKIP_BENCH=1 bash tests/fafafa.core.simd/docker/run_riscvv_opcode_lane.sh`：PASS，`[TEST] OK`、`[LEAK] OK`、`[RVV-LANE] PASS`
  - `TMPDIR=/home/dtamade/projects/fafafa.core/.claude/worktrees/simd-external-evidence/.simd-output/tmp FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=/home/dtamade/projects/fafafa.core/.claude/worktrees/simd-external-evidence/.simd-output/postfix-rvv-prebuilt-gate-20260327 bash tests/fafafa.core.simd/BuildOrTest.sh gate`：PASS，最终 `[GATE] OK`
- 这轮结论：
  - RVV opcode-ready lane 现在不仅 compile/runtime define 对齐，连 compile-only / suite / bench 的默认编译器口径也对齐了
  - 后续如果有人再次把 suite/bench 默认退回非 prebuilt compiler，默认 `check` 会先打红，不会等到 QEMU/RVV lane 才暴露

## Latest Findings (Phase 106)
- 这轮 fresh 审查没有再打出新的 SIMD 生产代码 bug，当前 active `SIMD-B23(candidate)` 的真实问题面已经收缩成证据链而不是实现层：
  - `FAFAFA_BUILD_MODE=Release ... BuildOrTest.sh check`：PASS
  - `FAFAFA_BUILD_MODE=Release ... BuildOrTest.sh gate`：PASS
  - 说明当前 worktree 的本地 `Release` 主线仍是绿的
- 初次 fresh `freeze-status` 的红点有两类：
  - Linux/QEMU CPUInfo non-x86 evidence 被当前 gate profile 跳过：`linux_qemu_cpuinfo_nonx86_evidence=SKIP`
  - Windows evidence 仍停留在 `2026-03-24 13:55:04`，已经超过 72 小时且早于最新 `src/fafafa.core.simd.pas`
- Linux/QEMU 这半边已经在当前工作机 fresh 收口：
  - Docker 可用：`docker info --format '{{.ServerVersion}}'` 返回 `29.3.0`
  - 按 freeze 建议跑：
    - `FAFAFA_BUILD_MODE=Release SIMD_OUTPUT_ROOT=.../review-20260327-qemu-gate SIMD_QEMU_PLATFORMS='linux/arm/v7 linux/arm64 linux/riscv64' SIMD_GATE_QEMU_NONX86_EVIDENCE=0 SIMD_GATE_QEMU_CPUINFO_NONX86_EVIDENCE=1 SIMD_GATE_QEMU_CPUINFO_NONX86_FULL_EVIDENCE=0 SIMD_GATE_QEMU_CPUINFO_NONX86_FULL_REPEAT=0 SIMD_GATE_QEMU_ARCH_MATRIX_EVIDENCE=0 bash tests/fafafa.core.simd/BuildOrTest.sh gate`
  - 结果：PASS，`qemu-cpuinfo-nonx86-evidence` 对 `linux/arm/v7`、`linux/arm64`、`linux/riscv64` 全 PASS
  - fresh `freeze-status` 随后转成：
    - Linux mainline required steps 全 PASS
    - cross gate required steps 全 PASS
    - 仅剩 Windows stale evidence 两个失败项
- Windows 这半边不是脚本失效，也不是权限缺失：
  - `FAFAFA_BUILD_MODE=Release bash tests/fafafa.core.simd/BuildOrTest.sh win-evidence-preflight`：PASS
  - `gh auth status`：PASS，账号 `dtamade` 已登录，scope 包含 `workflow`
  - 当前 repo tracking 正常：`git rev-list --left-right --count @{u}...HEAD` 为 `0 0`
- 真正的剩余 blocker 是 dispatch 条件，而不是 preflight：
  - `tests/fafafa.core.simd/run_windows_b07_closeout_via_github_actions.sh` 在未传 `run-id` 时会先检查本地 worktree 是否 dirty
  - 现态 `git status --short --untracked-files=no` 非空，且包含 `src/fafafa.core.simd.pas`、`src/fafafa.core.simd.riscvv.pas`、`src/fafafa.core.simd.riscvv.register.inc` 与多条 test 改动
  - 因此当前不能诚实地 dispatch 一轮 fresh Windows workflow 去代表本地未提交 SIMD 状态；必须先：
    - 提交并推送当前 SIMD 改动，再跑 `win-evidence-via-gh`
    - 或改走真实 Windows 主机执行 `tests\\fafafa.core.simd\\buildOrTest.bat evidence-win-verify`
- 这轮结论：
  - `SIMD-B23(candidate)` 已经完成 Linux 侧证据刷新
  - 剩余阻塞是“当前 dirty worktree 的 Windows evidence 该如何生成”，这是操作面决策，不是仓库里又出现了新的待修代码缺陷
