unit Contracts_Factories_TE_Impl;

{$CODEPAGE UTF8}
{$mode objfpc}{$H+}

{ TE（类型擦除）版契约工厂：桥接现有具体类型到扩展接口集（含批量/能力查询） }
...
type
  { 具体包装：将现有类型桥接到 IQueueInt/IStackInt/IMapIntStr }
...
function TQueueInt_SPSC.TryPeek(out AValue: Integer): Boolean; begin AValue := 0; Exit(False); end;
...
function TQueueInt_SPSC.EnqueueMany(const Values: array of Integer): Integer;
var i: Integer;
begin
  Result := 0;
  for i := Low(Values) to High(Values) do
    if FQ.Enqueue(Values[i]) then Inc(Result) else Break;
end;
function TQueueInt_SPSC.DequeueMany(var Values: array of Integer): Integer;
var i: Integer;
begin
  Result := 0;
  for i := Low(Values) to High(Values) do
    if FQ.Dequeue(Values[i]) then Inc(Result) else Break;
end;
procedure TQueueInt_SPSC.Clear;
var x: Integer;
begin
  while FQ.Dequeue(x) do ;
end;
...
function TQueueInt_MPSC.EnqueueMany(const Values: array of Integer): Integer;
var i: Integer;
begin
  Result := 0;
  for i := Low(Values) to High(Values) do begin FQ.Enqueue(Values[i]); Inc(Result); end;
end;
function TQueueInt_MPSC.DequeueMany(var Values: array of Integer): Integer;
var i: Integer;
begin
  Result := 0;
  for i := Low(Values) to High(Values) do if FQ.Dequeue(Values[i]) then Inc(Result) else Break;
end;
procedure TQueueInt_MPSC.Clear;
var x: Integer;
begin
  while FQ.Dequeue(x) do ;
end;
...
procedure TQueueInt_MPMC.Clear;
var x: Integer;
begin
  while FQ.Dequeue(x) do ;
end;
...
function TStackInt_Treiber.PushMany(const Values: array of Integer): Integer;
var i: Integer;
begin
  Result := 0; for i := Low(Values) to High(Values) do begin FS.Push(Values[i]); Inc(Result); end;
end;
function TStackInt_Treiber.PopMany(var Values: array of Integer): Integer;
var i: Integer;
begin
  Result := 0; for i := Low(Values) to High(Values) do if FS.Pop(Values[i]) then Inc(Result) else Break;
end;
procedure TStackInt_Treiber.Clear;
var x: Integer;
begin
  while FS.Pop(x) do ;
end;
...
function TStackInt_PreAlloc.PushMany(const Values: array of Integer): Integer;
var i: Integer; ok: Boolean;
begin
  Result := 0; for i := Low(Values) to High(Values) do begin ok := FS.Push(Values[i]); if ok then Inc(Result) else Break; end;
end;
function TStackInt_PreAlloc.PopMany(var Values: array of Integer): Integer;
var i: Integer;
begin
  Result := 0; for i := Low(Values) to High(Values) do if FS.Pop(Values[i]) then Inc(Result) else Break;
end;
procedure TStackInt_PreAlloc.Clear;
var x: Integer;
begin
  while FS.Pop(x) do ;
end;
...
function TMap_IntStr_OA.Put(AKey: Integer; const AValue: string; out Replaced: Boolean): Boolean;
var tmp: string;
begin
...
function TMap_IntStr_OA.Remove(AKey: Integer; out OldValue: string): Boolean;
begin
...
function TMap_IntStr_MM.Remove(AKey: Integer; out OldValue: string): Boolean;
begin
  if FM.find(AKey, OldValue) then Exit(FM.remove(AKey)) else Exit(False);
end;
function TMap_IntStr_MM.ContainsKey(AKey: Integer): Boolean; begin Result := FM.contains(AKey); end;
procedure TMap_IntStr_MM.Clear; begin FM.clear; end;
function TMap_IntStr_MM.Size: SizeInt; begin Result := FM.size; end;
function TMap_IntStr_MM.LoadFactorTimes1000: Integer;
begin
...
function TMap_IntStr_MM.SetMaxLoadFactorTimes1000(Value: Integer): Boolean;
begin
...
function TQueueInt_SPSC.Enqueue(AValue: Integer): Boolean;
begin
  Result := FQ.Enqueue(AValue);
end;

function TQueueInt_SPSC.TryDequeue(out AValue: Integer): Boolean;
begin
  Result := FQ.Dequeue(AValue);
end;

function TQueueInt_SPSC.TryPeek(out AValue: Integer): Boolean;
begin
  AValue := 0;
  Result := False;
end;

function TQueueInt_SPSC.IsEmpty: Boolean;
begin
  Result := FQ.IsEmpty;
end;

function TQueueInt_SPSC.Size: SizeInt;
begin
  Result := FQ.Size;
end;

function TQueueInt_SPSC.Capacity: SizeInt;
begin
  Result := FQ.Capacity;
end;

function TQueueInt_SPSC.Bounded: Boolean;
begin
  Result := True;
end;
...
function TQueueInt_MPSC.Enqueue(AValue: Integer): Boolean;
begin
  FQ.Enqueue(AValue);
  Result := True;
end;

function TQueueInt_MPSC.TryDequeue(out AValue: Integer): Boolean;
begin
  Result := FQ.Dequeue(AValue);
end;

function TQueueInt_MPSC.TryPeek(out AValue: Integer): Boolean;
begin
  AValue := 0;
  Result := False;
end;

function TQueueInt_MPSC.IsEmpty: Boolean;
begin
  Result := FQ.IsEmpty;
end;

function TQueueInt_MPSC.Size: SizeInt;
begin
  Result := 0; // 未提供 Size，返回 0（最佳努力）
end;

function TQueueInt_MPSC.Capacity: SizeInt;
begin
  Result := -1; // 无界
end;

function TQueueInt_MPSC.Bounded: Boolean;
begin
  Result := False;
end;
...
function TQueueInt_MPMC.Enqueue(AValue: Integer): Boolean;
begin
  Result := FQ.Enqueue(AValue);
end;

function TQueueInt_MPMC.TryDequeue(out AValue: Integer): Boolean;
begin
  Result := FQ.Dequeue(AValue);
end;

function TQueueInt_MPMC.TryPeek(out AValue: Integer): Boolean;
begin
  Result := FQ.TryPeek(AValue);
end;

function TQueueInt_MPMC.IsEmpty: Boolean;
begin
  Result := FQ.IsEmpty;
end;

function TQueueInt_MPMC.Size: SizeInt;
begin
  Result := FQ.GetSize;
end;

function TQueueInt_MPMC.Capacity: SizeInt;
begin
  Result := FQ.GetCapacity;
end;

function TQueueInt_MPMC.Bounded: Boolean;
begin
  Result := True;
end;
...
function TStackInt_Treiber.Push(AValue: Integer): Boolean;
begin
  FS.Push(AValue);
  Result := True;
end;

function TStackInt_Treiber.TryPop(out AValue: Integer): Boolean;
begin
  Result := FS.Pop(AValue);
end;

function TStackInt_Treiber.TryPeek(out AValue: Integer): Boolean;
begin
  Result := FS.TryPeek(AValue);
end;

function TStackInt_Treiber.IsEmpty: Boolean;
begin
  Result := FS.IsEmpty;
end;

function TStackInt_Treiber.Size: SizeInt;
begin
  Result := FS.GetSize;
end;

function TStackInt_Treiber.Capacity: SizeInt;
begin
  Result := -1;
end;

function TStackInt_Treiber.Bounded: Boolean;
begin
  Result := False;
end;
...
function TStackInt_PreAlloc.Push(AValue: Integer): Boolean;
begin
  Result := FS.Push(AValue);
end;

function TStackInt_PreAlloc.TryPop(out AValue: Integer): Boolean;
begin
  Result := FS.Pop(AValue);
end;

function TStackInt_PreAlloc.TryPeek(out AValue: Integer): Boolean;
begin
  // 预分配栈未提供 TryPeek，统一返回 False
  AValue := 0;
  Result := False;
end;

function TStackInt_PreAlloc.IsEmpty: Boolean;
begin
  Result := FS.IsEmpty;
end;

function TStackInt_PreAlloc.Size: SizeInt;
begin
  Result := FS.GetSize;
end;

function TStackInt_PreAlloc.Capacity: SizeInt;
begin
  Result := FS.GetCapacity;
end;

function TStackInt_PreAlloc.Bounded: Boolean;
begin
  Result := True;
end;
...
var
  LOld: string;
begin
  // OA 的 Put 返回 True 表示插入或覆盖；无法区分覆盖，采用双查策略
  Replaced := False;
  if FM.ContainsKey(AKey) then
  begin
    Replaced := True;
  end;
  Result := FM.Put(AKey, AValue);
...
function TMap_IntStr_OA.TryGetValue(AKey: Integer; out AValue: string): Boolean;
begin
...
var
  LHas: Boolean;
begin
  LHas := FM.Get(AKey, OldValue);
  Result := FM.Remove(AKey);
  if Result and (not LHas) then
    OldValue := '';
end;

function TMap_IntStr_OA.ContainsKey(AKey: Integer): Boolean;
begin
  Result := FM.ContainsKey(AKey);
end;

procedure TMap_IntStr_OA.Clear;
begin
  FM.Clear;
end;

function TMap_IntStr_OA.Size: SizeInt;
begin
  Result := FM.GetSize;
end;
...
var
  LDummy: string;
begin
  // MM insert 返回 True 表示插入；覆盖需要先 Remove 再 insert 或者提供 update
  // 这里采用 find+逻辑：若 find 命中视为将覆盖（目前实现 insert 逻辑删除旧值/新建）
  Replaced := FM.find(AKey, LDummy);
  Result := FM.insert(AKey, AValue);
end;

function TMap_IntStr_MM.TryGetValue(AKey: Integer; out AValue: string): Boolean;
begin
...
function TMap_IntStr_MM.Remove(AKey: Integer; out OldValue: string): Boolean;
begin
  // MM map remove 接口语义与 OA 不同；现实现提供 remove 返回布尔，无旧值
  // 这里先 find，再 remove
  if FM.find(AKey, OldValue) then
    Exit(FM.remove(AKey))
  else
    Exit(False);
end;

function TMap_IntStr_MM.ContainsKey(AKey: Integer): Boolean;
begin
  Result := FM.contains(AKey);
end;

procedure TMap_IntStr_MM.Clear;
begin
  FM.clear;
end;

function TMap_IntStr_MM.Size: SizeInt;
begin
  Result := FM.size;
end;
...
function TQueueFactory_Integer_TE.MakeSPSC(const ACapacity: SizeInt): IQueueInt;
begin
  Result := TQueueInt_SPSC.Create(ACapacity);
end;

function TQueueFactory_Integer_TE.MakeMPSC: IQueueInt;
begin
  Result := TQueueInt_MPSC.Create;
end;

function TQueueFactory_Integer_TE.MakeMPMC(const ACapacity: SizeInt): IQueueInt;
begin
  Result := TQueueInt_MPMC.Create(ACapacity);
end;

function TStackFactory_Integer_TE.MakeTreiber: IStackInt;
begin
  Result := TStackInt_Treiber.Create;
end;

function TStackFactory_Integer_TE.MakePreAlloc(const ACapacity: SizeInt): IStackInt;
begin
  Result := TStackInt_PreAlloc.Create(ACapacity);
end;

function TMapFactory_IntStr_TE.MakeOA(const ACapacity: SizeInt): IMapIntStr;
begin
  Result := TMap_IntStr_OA.Create(ACapacity);
end;

function TMapFactory_IntStr_TE.MakeMM(const ABucketCount: SizeInt): IMapIntStr;
begin
  Result := TMap_IntStr_MM.Create(ABucketCount);
end;

// 获取默认工厂
function GetDefaultQueueFactory_Integer_TE: IQueueFactory_Integer;
begin
  Result := TQueueFactory_Integer_TE.Create;
end;

function GetDefaultStackFactory_Integer_TE: IStackFactory_Integer;
begin
  Result := TStackFactory_Integer_TE.Create;
end;

function GetDefaultMapFactory_IntStr_TE: IMapFactory_IntStr;
begin
  Result := TMapFactory_IntStr_TE.Create;
end;
