...
REM Summarize latest minimal-runner log
REM Usage: Summarize_Minimal_Runner.bat [log_file] [tail_lines]
...
if "%LOG_FILE%"=="" set "LOG_FILE=%LOG_DIR%\latest_minimal_runner.log"
if "%TAIL_LINES%"=="" set "TAIL_LINES=80"

if not exist "%LOG_FILE%" (
  echo [error] Log file not found: %LOG_FILE%
  echo        Please run: BuildOrTest.bat minimal-runner
  exit /b 1
)
...
for %%K in (suite minimal map queue stack hash) do (
  echo ---------------- FINDSTR: %%K ----------------
