...
CPUINFO_X86_BATCH_RUNNER="${REPO_ROOT}/tests/fafafa.core.simd.cpuinfo.x86/buildOrTest.bat"
...
if [[ ! -f "${CPUINFO_X86_BATCH_RUNNER}" ]]; then
  fail "missing cpuinfo.x86 batch runner: ${CPUINFO_X86_BATCH_RUNNER}"
fi
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/simd-cpuinfo-x86-batch-success-XXXXXX")"
...
Info: (lazarus) Execute Title="Compile Project, Mode: Release, Target: C:\\repo\\tests\\fafafa.core.simd.cpuinfo.x86\\bin\\fafafa.core.simd.cpuinfo.x86.test.exe"
(9015) Linking ${WIN_TMP_DIR}\\success-out\\bin\\fafafa.core.simd.cpuinfo.x86.test.exe
...
if [[ ${SUCCESS_RC} -ne 0 ]]; then
  printf '%s\n' "${SUCCESS_OUTPUT}" >&2
  fail "synthetic cpuinfo.x86 compiled-summary batch case should succeed"
fi

if ! printf '%s' "${SUCCESS_OUTPUT}" | rg -n "\[BUILD\] WARN .*compile/link summary is present" >/dev/null; then
  printf '%s\n' "${SUCCESS_OUTPUT}" >&2
  fail "synthetic cpuinfo.x86 compiled-summary batch case did not emit the expected warn marker"
fi

if ! printf '%s' "${SUCCESS_OUTPUT}" | rg -n "\[BUILD\] OK" >/dev/null; then
  printf '%s\n' "${SUCCESS_OUTPUT}" >&2
  fail "synthetic cpuinfo.x86 compiled-summary batch case did not finish with BUILD OK"
fi
...
echo "[PASS] windows SIMD cpuinfo.x86 batch build success criteria verified"
