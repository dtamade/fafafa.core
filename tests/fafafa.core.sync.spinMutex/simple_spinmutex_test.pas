...
begin
  ...
  try
    ...
    Mutex := CreateSpinMutex('test_mutex');
    if Mutex = nil then
    begin
    ...
    // 高性能配置
    Mutex := CreateHighPerformanceSpinMutex('high_perf_mutex');
    Config := Mutex.GetConfig;
    WriteLn('  高性能配置:');
    WriteLn('    MaxSpinCount: ', Config.MaxSpinCount);
    WriteLn('    BackoffStrategy: ', Ord(Config.BackoffStrategy));
    ...
    // 低延迟配置
    Mutex := CreateLowLatencySpinMutex('low_latency_mutex');
    Config := Mutex.GetConfig;
    WriteLn('  低延迟配置:');
    WriteLn('    MaxSpinCount: ', Config.MaxSpinCount);
    WriteLn('    BackoffStrategy: ', Ord(Config.BackoffStrategy));
    ...
    // 测试 5: 超时测试
    WriteLn('测试 5: 超时测试');
    Mutex := CreateSpinMutex('timeout_test');
    ...
    // 尝试带超时的获取（应该失败）
    var TimeoutGuard := Mutex.TryLockFor(100, 'timeout_attempt');
