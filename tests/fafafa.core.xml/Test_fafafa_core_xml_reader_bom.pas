...
begin
  ...
  try
    ...
    MS.WriteBuffer(Pointer('<r>t</r>')^, Length('<r>t</r>'));
    MS.Position := 0;
...
begin
  ...
  try
    ...
    MS.WriteBuffer(Pointer('<r/>')^, Length('<r/>'));
    MS.Position := 0;
...
procedure Test_UTF32_Default_Unsupported_AutoDecode_OK_Internal(const WriterProc: procedure(const S: RawByteString; MS: TStream));
...
begin
  ...
  try
    ...
    // AutoDecode：应成功
    MS.Position := 0; SetLength(Vals, 0);
    R := CreateXmlReader.ReadFromStream(MS, [xrfCoalesceText, xrfAutoDecodeEncoding]);
    while R.Read do begin i := Length(Vals); SetLength(Vals, i+1); Vals[i] := R.Token; end;
...
begin
  Test_UTF32_Default_Unsupported_AutoDecode_OK_Internal(@WriteUTF32LE_WithBOM);
end;

procedure TTestCase_Reader_BOM.Test_UTF32_BOM_Default_Unsupported_AutoDecode_OK_BE;
begin
  Test_UTF32_Default_Unsupported_AutoDecode_OK_Internal(@WriteUTF32BE_WithBOM);
end;
