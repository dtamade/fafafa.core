...
set "PROJ=%ROOT%fafafa.core.simd.cpuinfo.test.lpr"
set "FPC_EXE=%FPC_BIN%"
...
set "TARGET_TEST_LOG=%TARGET_LOG_DIR%\test.txt"
...
echo Isolation env: SIMD_OUTPUT_ROOT=C:\temp\simd-cpuinfo-run-123 ^(override bin/lib/logs root^)
exit /b 2
...
if not exist "%LOG_DIR%" mkdir "%LOG_DIR%"
if not exist "%TARGET_LOG_DIR%" mkdir "%TARGET_LOG_DIR%"
"%FPC_EXE%" -Fu"%SRC_DIR%" -Fi"%SRC_DIR%" -FE"%BIN_DIR%" -FU"%LIB_DIR%" -o"%BIN%" "%PROJ%" > "%BUILD_LOG%" 2>&1
if errorlevel 1 (
...
copy /y "%TEST_LOG%" "%TARGET_TEST_LOG%" >nul 2>nul
if not "%RUN_RC%"=="0" (
