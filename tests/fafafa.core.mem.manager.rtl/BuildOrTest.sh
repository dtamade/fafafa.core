...
PROJ="$ROOT/fafafa.core.mem.manager.rtl.test.lpi"
BIN="$ROOT/bin/fafafa.core.mem.manager.rtl.test_debug"
BIN_FALLBACK="$ROOT/bin/fafafa.core.mem.manager.rtl.test"
LOG_DIR="$ROOT/logs"
BUILD_LOG="$LOG_DIR/build.txt"
TEST_LOG="$LOG_DIR/test.txt"
TOOLS_LAZBUILD="$REPO_ROOT/tools/lazbuild.sh"
MODULE_WARNING_PATTERN='(^|.*/)src/fafafa\.core\.mem\.(manager\.rtl|allocator\.(rtlAllocator|base))[^[:space:]]*.*(Warning:|Hint:)'
