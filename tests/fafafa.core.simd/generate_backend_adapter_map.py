#!/usr/bin/env python3
"""Generate fafafa.core.simd.backend.adapter.map.inc from the CSV spec."""
...
def default_spec_path() -> Path:
    return repo_root() / "src" / "fafafa.core.simd.backend.adapter.map.csv"


def default_output_path() -> Path:
    return repo_root() / "src" / "fafafa.core.simd.backend.adapter.map.inc"
...
def render_include_text(a_rows: list[AdapterMappingRow]) -> str:
    l_lines = [
        "{",
        "  Generated from fafafa.core.simd.backend.adapter.map.csv.",
