#!/usr/bin/env python3
"""Guard SIMD dataplane consumer helpers from leaking past companion surfaces."""
...
SUMMARY_PREFIX = "DATAPLANE_CONSUMER_SCOPE"
SCAN_GLOBS = (
...
def parse_args() -> argparse.Namespace:
    l_parser = argparse.ArgumentParser(
        description=(
            "Check that dataplane consumer helpers only stay in "
            "dataplane/direct/public ABI/facade companion surfaces."
        )
    )
...
def print_human_report(a_report: Report, a_verbose: bool) -> None:
    if a_report.forbidden_hits:
        for l_hit in a_report.forbidden_hits:
            print(
                "[DATAPLANE-SCOPE] FORBIDDEN "
                f"{l_hit.path}:{l_hit.line}: {l_hit.symbol}: {l_hit.text}"
            )
    elif a_verbose:
        print("[DATAPLANE-SCOPE] No forbidden dataplane consumer helper usage found.")

    if a_verbose:
        for l_hit in a_report.allowed_hits:
            print(
                "[DATAPLANE-SCOPE] allowed "
                f"{l_hit.path}:{l_hit.line}: {l_hit.symbol}: {l_hit.text}"
            )

    if a_report.ok:
        print("[DATAPLANE-SCOPE] OK")
    else:
        print(
            "[DATAPLANE-SCOPE] FAILED: dataplane consumer helpers must stay in "
            "dataplane/direct/public ABI/facade companion surfaces so other "
            "modules cannot grow a second publication-consumer path."
        )
...
def main() -> int:
    ...
    if not l_root.exists():
        print(f"[DATAPLANE-SCOPE] Missing root: {l_root}")
        return 2
    ...
    if l_args.json_file:
        ...
        print(f"[DATAPLANE-SCOPE] JSON snapshot: {l_json_path}")
