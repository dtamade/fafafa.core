...
mkdir -p "${LCaseApply}/tests/fafafa.core.simd/docs" "${LCaseApply}/tests/fafafa.core.simd/logs" "${LCaseApply}/docs/plans"
cp "${SNIPPET_SCRIPT}" "${LCaseApply}/tests/fafafa.core.simd/apply_windows_b07_closeout_updates.sh"
chmod +x "${LCaseApply}/tests/fafafa.core.simd/apply_windows_b07_closeout_updates.sh"

cat > "${LCaseApply}/tests/fafafa.core.simd/verify_windows_b07_evidence.sh" <<'EOM'
#!/usr/bin/env bash
exit 0
EOM
chmod +x "${LCaseApply}/tests/fafafa.core.simd/verify_windows_b07_evidence.sh"

cat > "${LCaseApply}/tests/fafafa.core.simd/logs/windows_b07_gate.log" <<'EOM'
[B07] Windows evidence capture
[GATE] OK
EOM

cat > "${LCaseApply}/tests/fafafa.core.simd/logs/freeze_status.json" <<'EOM'
{
  "mode": "cross-platform",
  "freeze_ready": true,
  "cross_ready": true
...
cat > "${LCaseApply}/tests/fafafa.core.simd/logs/windows_b07_closeout_summary.md" <<'EOM'
...
- Command: bash verify_windows_b07_evidence.sh "tests/fafafa.core.simd/logs/windows_b07_gate.log"
- Result: PASS
EOM
...
cat > "${LCaseApply}/tests/fafafa.core.simd/docs/simd_completeness_matrix.md" <<'EOM'
- Windows 证据：脚本入口 + 校验入口已就绪（待 Windows 实机日志）
- [~] Windows 证据脚本+校验器就绪（待实机执行产出）
EOM

cat > "${LCaseApply}/tests/fafafa.core.simd/docs/simd_release_candidate_checklist.md" <<'EOM'
...
"${LCaseApply}/tests/fafafa.core.simd/apply_windows_b07_closeout_updates.sh" \
  "${LCaseApply}/tests/fafafa.core.simd/logs/windows_b07_closeout_summary.md" \
  --apply \
  --freeze-json "${LCaseApply}/tests/fafafa.core.simd/logs/freeze_status.json" \
  --target-root "${LCaseApply}" \
  --batch-id SIMD-APPLY-20260517 > "${LCaseApply}/apply_stdout.txt"

if ! grep -F -- 'Windows 实机证据日志曾归档（历史批次）' "${LCaseApply}/tests/fafafa.core.simd/docs/simd_release_candidate_checklist.md" >/dev/null; then
  echo "[WIN-CLOSEOUT-SUMMARY-REHEARSAL] FAILED: apply case should keep RC historical wording"
  cat "${LCaseApply}/tests/fafafa.core.simd/docs/simd_release_candidate_checklist.md"
  exit 1
fi

if ! grep -F -- 'Windows 实机证据曾归档（历史批次；脚本+校验器+日志）' "${LCaseApply}/tests/fafafa.core.simd/docs/simd_completeness_matrix.md" >/dev/null; then
  echo "[WIN-CLOSEOUT-SUMMARY-REHEARSAL] FAILED: apply case should keep matrix historical wording"
  cat "${LCaseApply}/tests/fafafa.core.simd/docs/simd_completeness_matrix.md"
  exit 1
fi

if ! grep -F -- '当前 `HEAD` 是否完成跨平台证据闭环，仍以最新 `freeze-status` 为准。' "${LCaseApply}/tests/fafafa.core.simd/docs/simd_release_candidate_checklist.md" >/dev/null; then
