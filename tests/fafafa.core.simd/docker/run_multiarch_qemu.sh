...
case "${SCENARIO}" in
  ...
  cpuinfo-nonx86-evidence)
    CONTAINER_CMD='FAFAFA_BUILD_MODE=Release SIMD_CPUINFO_RUNTIME_COPY=1 bash tests/fafafa.core.simd.cpuinfo/BuildOrTest.sh check && FAFAFA_BUILD_MODE=Release SIMD_CPUINFO_RUNTIME_COPY=1 bash tests/fafafa.core.simd.cpuinfo/BuildOrTest.sh test --list-suites && LTARGET_CPU="$(fpc -iTP 2>/dev/null | tr "[:upper:]" "[:lower:]" || true)" && LTARGET_OS="$(fpc -iTO 2>/dev/null | tr "[:upper:]" "[:lower:]" || true)" && LTARGET_LOG="tests/fafafa.core.simd.cpuinfo/logs/${LTARGET_CPU:-unknowncpu}-${LTARGET_OS:-unknownos}/test.txt" && LSUITES="$(cat "$LTARGET_LOG" 2>/dev/null || cat tests/fafafa.core.simd.cpuinfo/logs/test.txt 2>/dev/null || true)" && if printf "%s\n" "$LSUITES" | grep -q "TTestCase_LazyCPUInfo"; then FAFAFA_BUILD_MODE=Release SIMD_CPUINFO_RUNTIME_COPY=1 bash tests/fafafa.core.simd.cpuinfo/BuildOrTest.sh test --suite=TTestCase_LazyCPUInfo; else echo "[TEST] SKIP lazy cpuinfo suite (not available on this target)"; fi && FAFAFA_BUILD_MODE=Release SIMD_CPUINFO_RUNTIME_COPY=1 bash tests/fafafa.core.simd.cpuinfo/BuildOrTest.sh test --suite=TTestCase_PlatformSpecific'
    ;;
  cpuinfo-nonx86-full-evidence)
    CONTAINER_CMD='FAFAFA_BUILD_MODE=Release SIMD_CPUINFO_RUNTIME_COPY=1 bash tests/fafafa.core.simd.cpuinfo/BuildOrTest.sh check && FAFAFA_BUILD_MODE=Release SIMD_CPUINFO_RUNTIME_COPY=1 bash tests/fafafa.core.simd.cpuinfo/BuildOrTest.sh test'
    ;;
  cpuinfo-nonx86-full-repeat)
    ...
    CONTAINER_CMD="FAFAFA_BUILD_MODE=Release SIMD_CPUINFO_RUNTIME_COPY=1 bash tests/fafafa.core.simd.cpuinfo/BuildOrTest.sh check && LROUNDS=${CPUINFO_REPEAT_ROUNDS} && for LRound in \$(seq 1 \"\$LROUNDS\"); do echo \"[REPEAT] \${LRound}/\${LROUNDS} cpuinfo-full\"; FAFAFA_BUILD_MODE=Release SIMD_CPUINFO_RUNTIME_COPY=1 bash tests/fafafa.core.simd.cpuinfo/BuildOrTest.sh test; done"
    ;;
