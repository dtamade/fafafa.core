...
updates = [
    ...
    (matrix, [
        ...
        ('- [~] Windows 证据脚本+校验器就绪（待实机执行产出）', '- [x] Windows 实机证据曾归档（历史批次；脚本+校验器+日志）'),
    ]),
    (rc, [
        ('- [ ] Windows 实机证据日志已归档（当前缺口）', '- [x] Windows 实机证据日志曾归档（历史批次）'),
        ('- Windows 侧：待补实机日志后完成跨平台证据闭环。', '- Windows 侧：历史实机日志已归档；当前 `HEAD` 是否完成跨平台证据闭环，仍以最新 `freeze-status` 为准。'),
    ]),
]
