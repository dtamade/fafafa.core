...
resolve_repo_slug_for_current_checkout() {
  local LRepoJson
  local LRemoteUrl
  local LRepo
...
PY
  ...
  if command -v git >/dev/null 2>&1; then
    ...
    if [[ -n "${LRemoteUrl}" ]]; then
      LRepo="$(extract_repo_slug_from_github_url "${LRemoteUrl}")"
      if [[ -n "${LRepo}" ]]; then
        printf '%s\n' "${LRepo}"
        return 0
      fi
...
resolve_required_self_hosted_runner_labels() {
  local aBackend

  aBackend="${1:-}"
  if [[ -n "${SIMD_NATIVE_EVIDENCE_REQUIRED_RUNNER_LABELS:-}" ]]; then
    printf '%s\n' "${SIMD_NATIVE_EVIDENCE_REQUIRED_RUNNER_LABELS}"
    return 0
  fi
...
format_runner_labels_for_output() {
  local aEncodedLabels

  aEncodedLabels="${1:-}"
  if [[ -z "${aEncodedLabels}" ]]; then
    return 0
  fi

  printf '%s\n' "${aEncodedLabels}" | tr '|' '\n' | sed 's/^[[:space:]]*//;s/[[:space:]]*$//' | awk 'NF' | paste -sd',' - | sed 's/,/, /g'
}

query_matching_self_hosted_runner_counts() {
  local aRepo
  local aEncodedLabels
  local LRunnersJson
  ...
  aRepo="${1:-}"
  aEncodedLabels="${2:-}"
  if [[ -z "${aRepo}" || -z "${aEncodedLabels}" ]]; then
    return 10
  fi

  LRunnersJson="$(gh api "repos/${aRepo}/actions/runners" 2>/dev/null || true)"
  if [[ -z "${LRunnersJson}" ]]; then
    return 10
  fi

  python3 - "${LRunnersJson}" "${aEncodedLabels}" <<'PY'
import json
import sys
...
fail_if_repo_runner_inventory_missing() {
  local LRepo
  ...
  LRepo="${1:-}"
  LEncodedLabels="${2:-}"
  if [[ -z "${LRepo}" || -z "${LEncodedLabels}" ]]; then
    return 1
  fi

  if ! IFS=$'\t' read -r LVisibleRunners LMatchingRunners < <(query_matching_self_hosted_runner_counts "${LRepo}" "${LEncodedLabels}"); then
    return 1
  fi

  if [[ "${LMatchingRunners:-}" != "0" ]]; then
    return 1
  fi

  LLabelDisplay="$(format_runner_labels_for_output "${LEncodedLabels}")"
  echo "[NATIVE-EVIDENCE-GH] No matching self-hosted runner available for labels: ${LLabelDisplay}"
  echo "[NATIVE-EVIDENCE-GH] Repo runner inventory has no registered self-hosted runner covering those labels."
  if [[ -n "${LVisibleRunners:-}" ]]; then
    echo "[NATIVE-EVIDENCE-GH] Visible repo self-hosted runner count: ${LVisibleRunners}"
  fi
  return 0
}

preflight_dispatch_for_self_hosted_runner() {
  local aBackend
  local LRepo
  local LEncodedLabels
  local LLabelsRc
  ...
  set +e
  LEncodedLabels="$(resolve_required_self_hosted_runner_labels "${aBackend}")"
  LLabelsRc=$?
  set -e
  if [[ "${LLabelsRc}" == "10" ]]; then
    return 0
  fi
  if [[ "${LLabelsRc}" != "0" || -z "${LEncodedLabels}" ]]; then
    return 0
  fi

  LRepo="$(resolve_repo_slug_for_current_checkout || true)"
  if [[ -z "${LRepo}" ]]; then
    return 0
  fi

  if fail_if_repo_runner_inventory_missing "${LRepo}" "${LEncodedLabels}"; then
    echo "[NATIVE-EVIDENCE-GH] Refuse dispatch before workflow run; reuse an existing run-id or register the runner first."
    return 32
  fi
...
if [[ -z "${LRunId}" ]]; then
  ...
  preflight_dispatch_for_self_hosted_runner "${BACKEND}" || exit $?
