...
这个目录是 `fafafa.core.mem.allocator.foundation` 的当前测试入口。它负责说明 strict L0 allocator facade 的 root runner、Windows wrapper，以及它和 `tests/fafafa.core.mem/`、`tests/fafafa.core.mem.manager.rtl/` 之间的边界。
...
5. `tests/fafafa.core.mem.allocator.foundation/BuildOrTest.sh`
6. `tests/fafafa.core.mem.allocator.foundation/BuildOrTest.bat`
7. `tests/fafafa.core.mem.allocator.foundation/buildOrTest.bat`
8. `tests/fafafa.core.mem.allocator.foundation/fafafa.core.mem.allocator.foundation.test.lpi`
...
- 这个目录只覆盖 strict L0 allocator facade，不负责 `mimalloc` / `crtAllocator` 这类可选后端集成测试。
