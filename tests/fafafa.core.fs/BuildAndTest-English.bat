...
set "PROJECT=%SCRIPT_DIR%tests_fs_no_fpcunit.lpi"
...
if /i "%1"=="test" (
  ...
  powershell -NoProfile -Command "& { & '%TEST_EXE%' --filter=fs.smoke; $code=$LastExitCode; exit $code }" | powershell -NoProfile -Command "$input | Out-File -FilePath '%SMOKE_LOG%' -Encoding utf8"
  set "RC=%ERRORLEVEL%"
  type "%SMOKE_LOG%"
  if not "%RC%"=="0" (
  ...
  powershell -NoProfile -Command "& { & '%TEST_EXE%' --filter=fs; $code=$LastExitCode; exit $code }" | powershell -NoProfile -Command "$input | Out-File -FilePath '%FULL_LOG%' -Encoding utf8"
  set "RC=%ERRORLEVEL%"
  powershell -NoProfile -Command "& { & '%TEST_EXE%' --filter=fs --junit='%JUNIT_XML%'; $code=$LastExitCode; exit $code }" >nul 2>&1
  type "%FULL_LOG%"
