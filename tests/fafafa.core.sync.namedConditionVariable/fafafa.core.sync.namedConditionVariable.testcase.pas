unit fafafa.core.sync.namedConditionVariable.testcase;
...
uses
  ...
  fafafa.core.base, fafafa.core.sync.namedConditionVariable, 
...
begin
  ...
  CheckNotNull(LCondVar2, '应该成功打开现有的命名条件变量');
...
begin
  ...
  CheckEquals(0, LStats.WaitCount, '空统计的等待次数应该为0');
end;
...
begin
  ...
  CheckTrue(LConfig.MaxWaiters > 0, '最大等待者数量应该大于0');
end;
...
var
  ...
  LResult: Boolean;
begin
  // 基本的等待-信号测试（使用超时避免无限等待）
  LGuard := FMutex.Lock;
  try
    // 使用很短的超时时间测试
    LResult := FCondVar.Wait(FMutex as ILock, 100); // 100毫秒超时
    CheckFalse(LResult, '没有信号时应该超时');
  finally
    LGuard := nil;
  end;
end;
...
var
  ...
  LResult: Boolean;
begin
  // 基本的等待-广播测试（使用超时避免无限等待）
  LGuard := FMutex.Lock;
  try
    // 使用很短的超时时间测试
    LResult := FCondVar.Wait(FMutex as ILock, 100); // 100毫秒超时
    CheckFalse(LResult, '没有广播时应该超时');
  finally
    LGuard := nil;
  end;
end;
...
var
  LResult: Boolean;
begin
  LResult := FCondVar.TryAcquire(1000);
  if LResult then
  try
    CheckTrue(True, '应该在超时内获取锁');
  finally
    FCondVar.Release;
  end;
end;
...
begin
  ...
  CheckNotNull(LCondVar2, '应该成功创建第二个实例');
  CheckEquals(FTestName, LCondVar2.GetName, '名称应该匹配');
  CheckFalse(LCondVar2.IsCreator, '第二个实例不应该是创建者');
end;
...
begin
  LConfig := DefaultNamedConditionVariableConfig;
  CheckTrue(LConfig.TimeoutMs > 0, '默认超时时间应该大于0');
  CheckTrue(LConfig.MaxWaiters > 0, '默认最大等待者数量应该大于0');
  CheckFalse(LConfig.UseGlobalNamespace, '默认不应该使用全局命名空间');
