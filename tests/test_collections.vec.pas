...
begin
  ...
  try
    ...
    try
      ...
      { 测试nil分配器自动使用RTL分配器 }
      LVec := specialize TVec<Integer>.Create(TAllocator(nil));
      AssertNotNull('Create with nil allocator should work', LVec);
      AssertNotNull('Create with nil allocator should use RTL allocator', LVec.GetAllocator);
      AssertTrue('Create with nil allocator should use RTL allocator', LVec.GetAllocator = GetRtlAllocator());
      AssertTrue('Vector should be empty', LVec.IsEmpty);
      AssertEquals('Vector count should be 0', Int64(0), Int64(LVec.GetCount));
    finally
...
begin
  ...
  try
    ...
    try
      ...
      { 测试nil分配器自动使用RTL分配器 }
      LVec := specialize TVec<Integer>.Create(TAllocator(nil), LTestData);
      AssertNotNull('Create with nil allocator should work', LVec);
      AssertNotNull('Create with nil allocator should use RTL allocator', LVec.GetAllocator);
      AssertTrue('Create with nil allocator should use RTL allocator', LVec.GetAllocator = GetRtlAllocator());
      AssertTrue('Vector should store provided data', LVec.GetData = LTestData);
  ...
  try
    ...
    try
      ...
      try
        ...
        AssertTrue('Vector should use provided grow strategy', LVec.GetGrowStrategy = LGrowStrategy);
        AssertTrue('Vector should be empty', LVec.IsEmpty);
        AssertEquals('Vector count should be 0', Int64(0), Int64(LVec.GetCount));
        AssertEquals('Vector capacity should be 0', Int64(0), Int64(LVec.GetCapacity));
        AssertTrue('Vector data should be nil', LVec.GetData = nil);
    ...
    try
      ...
      try
        ...
        try
          ...
          AssertTrue('Vector should use provided allocator', LTargetVec.GetAllocator = LAllocator);
          AssertTrue('Vector should use provided grow strategy', LTargetVec.GetGrowStrategy = LGrowStrategy);
...
begin
  { 测试获取增长策略 }
  LStrategy := TFixedGrowStrategy.Create(5);
  try
    LVec := specialize TVec<Integer>.Create(0, GetRtlAllocator(), LStrategy);
    try
      AssertNotNull('GrowStrategy should not be nil', LVec.GetGrowStrategy);
      AssertTrue('Should use provided strategy', LVec.GetGrowStrategy = LStrategy);
      AssertTrue('GrowStrategy property should work', LVec.GetGrowStrategy = LVec.GrowStrategy);
    finally
      LVec.Free;
    end;
  finally
...
begin
  ...
  try
    ...
    try
      ...
      AssertTrue('Should use first strategy', LVec.GetGrowStrategy = LStrategy1);

      { 测试通过属性设置 }
      LVec.GrowStrategy := LStrategy2;
      AssertTrue('Should use second strategy', LVec.GetGrowStrategy = LStrategy2);
    finally
      LVec.Free;
    end;
  finally
...
begin
  ...
  try
    ...
    AssertException(
      ...
      procedure
      begin
        LVec.Delete(High(SizeUInt), 1);  { 相当于-1 }
      end);
    {$ENDIF}
