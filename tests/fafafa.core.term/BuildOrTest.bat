...
set "PROJECT=fafafa.core.term.test.lpi"
set "TEST_EXECUTABLE=bin\fafafa.core.term.test.exe"
...
REM Ensure logs directory and default quiet mode for build output
if not exist "logs" mkdir "logs" >nul 2>nul
if not defined FAFAFA_BUILD_QUIET set "FAFAFA_BUILD_QUIET=1"
...
set "LAST_LOG=logs\last.txt"
if exist "%LAST_LOG%" del /q "%LAST_LOG%" >nul 2>nul
...
copy /y "%BUILD_LOG%" "%LAST_LOG%" >nul 2>nul
...
if /i "%CMD%"=="test" (
  if exist "%TEST_EXECUTABLE%" (
    ...
    if !EXIT_ERR! EQU 0 (
      ...
      echo [TEST] FAILED code=!EXIT_ERR!
