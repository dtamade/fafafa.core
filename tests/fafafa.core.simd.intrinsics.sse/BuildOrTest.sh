...
PROJ="${ROOT}/fafafa.core.simd.intrinsics.sse.test.lpi"
FPC_BIN="${FPC_BIN:-fpc}"
...
BIN="${BIN_DIR}/fafafa.core.simd.intrinsics.sse.test"
...
resolve_test_binary() {
  ...
  for LCandidate in \
    "${BIN}" \
    "${BIN}.exe" \
    "${ROOT}/bin/fafafa.core.simd.intrinsics.sse.test" \
    "${ROOT}/bin/fafafa.core.simd.intrinsics.sse.test.exe"; do
  ...
  while IFS= read -r LCandidate; do
    ...
    find "${OUTPUT_ROOT}" "${ROOT}" -maxdepth 3 -type f \
      \( -name 'fafafa.core.simd.intrinsics.sse.test' -o -name 'fafafa.core.simd.intrinsics.sse.test.exe' \) \
      2>/dev/null | sort -u
  )
