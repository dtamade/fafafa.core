...
set LPI=%EXAMPLE_DIR%example_csv_reporter.lpi
...
if not exist "%LPI%" (
  ...
  >>"%LPI%" echo     ^<General^>
  >>"%LPI%" echo       ^<Title Value="example_csv_reporter"/^>
  ...
  >>"%LPI%" echo           ^<Target^>
  >>"%LPI%" echo             ^<Filename Value=".\bin\example_csv_reporter"/^>
  >>"%LPI%" echo           ^</Target^>
  ...
  >>"%LPI%" echo     ^<Units^>
  >>"%LPI%" echo       ^<Unit^>
  >>"%LPI%" echo         ^<Filename Value="example_csv_reporter.lpr"/^>
...
"%BIN_DIR%example_csv_reporter.exe"
