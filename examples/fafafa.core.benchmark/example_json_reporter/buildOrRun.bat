...
set LPI=%EXAMPLE_DIR%example_json_reporter.lpi
...
if not exist "%LPI%" (
  ...
  >>"%LPI%" echo     ^<General^>
  >>"%LPI%" echo       ^<Title Value="example_json_reporter"/^>
  ...
  >>"%LPI%" echo           ^<Target^>
  >>"%LPI%" echo             ^<Filename Value=".\bin\example_json_reporter"/^>
  >>"%LPI%" echo           ^</Target^>
  ...
  >>"%LPI%" echo     ^<Units^>
  >>"%LPI%" echo       ^<Unit^>
  >>"%LPI%" echo         ^<Filename Value="example_json_reporter.lpr"/^>
...
"%BIN_DIR%example_json_reporter.exe"
