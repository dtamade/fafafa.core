...
set LPI=%EXAMPLE_DIR%example_predictive.lpi
...
if not exist "%LPI%" (
  ...
  >>"%LPI%" echo     ^<General^>
  >>"%LPI%" echo       ^<Title Value="example_predictive"/^>
  ...
  >>"%LPI%" echo           ^<Target^>
  >>"%LPI%" echo             ^<Filename Value=".\bin\example_predictive"/^>
  >>"%LPI%" echo           ^</Target^>
  ...
  >>"%LPI%" echo     ^<Units^>
  >>"%LPI%" echo       ^<Unit^>
  >>"%LPI%" echo         ^<Filename Value="example_predictive.lpr"/^>
...
"%BIN_DIR%example_predictive.exe"
