...
echo [BUILD] %LAZBUILD% example_producer_consumer.lpi ^(project default mode^)
call "%LAZBUILD%" "example_producer_consumer.lpi"
...
) else if exist "..\bin\example_producer_consumer" (
  echo [RUN] ..\bin\example_producer_consumer
  "..\bin\example_producer_consumer"
) else (
  echo [ERROR] Executable not found: ..\bin\example_producer_consumer[.exe]
  exit /b 100
)
