...
if exist "%BIN_PATH%\example_fs.exe" (
    del /q "%BIN_PATH%\example_fs.exe" && echo Removed: example_fs.exe
)

echo.
echo Cleaning lib directory: %LIB_PATH%
if exist "%LIB_PATH%\*.o" (
    del /q "%LIB_PATH%\*.o" && echo Removed: object files
)
if exist "%LIB_PATH%\*.ppu" (
    del /q "%LIB_PATH%\*.ppu" && echo Removed: unit files
)
if exist "%LIB_PATH%\*.a" (
    del /q "%LIB_PATH%\*.a" && echo Removed: library files
)
