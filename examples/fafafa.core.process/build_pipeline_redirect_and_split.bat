...
%FPC_EXE% -MObjFPC -Scaghi -CirotR -O1 -gw3 -gl -gh -Xg -gt -vewnhibq ^
  ...
  -FU"%CD%\%LIBDIR%" ^
  -FE"%CD%\%OUTDIR%" ^
  -o"%CD%\%OUTDIR%\example_pipeline_redirect_and_split.exe" ^
  example_pipeline_redirect_and_split.lpr

if errorlevel 1 (
  echo [ERROR] Build failed.
  exit /b 1
) else (
  echo [OK] Built: %OUTDIR%\example_pipeline_redirect_and_split.exe
)
