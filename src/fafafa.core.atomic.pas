...
begin
  Result := UInt64(atomic_fetch_and_64(PInt64(@aObj)^, PInt64(@aArg)^));
end;
{$ENDIF}
...
begin
  Result := UInt64(atomic_fetch_or_64(PInt64(@aObj)^, PInt64(@aArg)^));
end;
{$ENDIF}
...
begin
  Result := UInt64(atomic_fetch_xor_64(PInt64(@aObj)^, PInt64(@aArg)^));
end;
{$ENDIF}
...
{$ELSE}
const
  // 32-bit: use low TAG_BITS for tag, rest for pointer; true single-word atomics
  TAG_BITS = 2; // default; can be made configurable via settings.inc if needed
  TAG_MASK = (1 shl TAG_BITS) - 1;
...
begin
  {$IFDEF CPU64}
  Result := UInt16(aTaggedPtr shr 48);
  {$ELSE}
...
begin
  InterlockedExchange64(PInt64(@aObj)^, PInt64(@aDesired)^);
end;

function atomic_tagged_ptr_exchange(var aObj: atomic_tagged_ptr_t; aDesired: atomic_tagged_ptr_t): atomic_tagged_ptr_t;
begin
  Result := atomic_tagged_ptr_t(InterlockedExchange64(PInt64(@aObj)^, PInt64(@aDesired)^));
end;
...
var
  LOld: Int64;
begin
  LOld := InterlockedCompareExchange64(PInt64(@aObj)^, PInt64(@aDesired)^, PInt64(@aExpected)^);
  Result := LOld = PInt64(@aExpected)^;
...
{$ELSE}
// 32-bit single-word lock-free impl using low TAG_BITS for tag
function atomic_tagged_ptr_load(var aObj: atomic_tagged_ptr_t): atomic_tagged_ptr_t;
begin
  Result := aObj; // single-word, atomic read sufficient for relaxed
end;

procedure atomic_tagged_ptr_store(var aObj: atomic_tagged_ptr_t; aDesired: atomic_tagged_ptr_t);
begin
  InterlockedExchange(PLongWord(@aObj)^, LongWord(aDesired));
end;

function atomic_tagged_ptr_exchange(var aObj: atomic_tagged_ptr_t; aDesired: atomic_tagged_ptr_t): atomic_tagged_ptr_t;
var oldv: LongWord;
begin
  oldv := InterlockedExchange(PLongWord(@aObj)^, LongWord(aDesired));
  Result := atomic_tagged_ptr_t(oldv);
end;

function atomic_tagged_ptr_compare_exchange_strong(var aObj: atomic_tagged_ptr_t; var aExpected: atomic_tagged_ptr_t; aDesired: atomic_tagged_ptr_t): Boolean;
var oldv: LongWord;
begin
  oldv := InterlockedCompareExchange(PLongWord(@aObj)^, LongWord(aDesired), LongWord(aExpected));
  Result := (oldv = LongWord(aExpected));
  if not Result then aExpected := atomic_tagged_ptr_t(oldv);
end;

function atomic_tagged_ptr_compare_exchange_weak(var aObj: atomic_tagged_ptr_t; var aExpected: atomic_tagged_ptr_t; aDesired: atomic_tagged_ptr_t): Boolean;
begin
  // identical to strong in this fallback; weak may spuriously fail in other impls
  Result := atomic_tagged_ptr_compare_exchange_strong(aObj, aExpected, aDesired);
end;
{$ENDIF}
