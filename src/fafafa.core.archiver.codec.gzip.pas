...
uses
  ...
  fafafa.core.archiver.codec.deflate.raw.paszlib;
...
var hdr: array[0..9] of Byte;
begin
  // ID1 ID2 CM FLG MTIME(4) XFL OS
  hdr[0] := $1F; hdr[1] := $8B; hdr[2] := 8; hdr[3] := 0; // CM=8(deflate), FLG=0
  hdr[4] := 0; hdr[5] := 0; hdr[6] := 0; hdr[7] := 0;     // MTIME=0 for deterministic
  hdr[8] := 0; // XFL (0: unknown/auto)
  hdr[9] := 255; // OS=255 unknown
  FDest.WriteBuffer(hdr, SizeOf(hdr));
end;

procedure TGZipEncodeStream.DoWriteTrailer;
var tr: array[0..7] of Byte; crc, isize: DWord;
begin
  // gzip CRC32 是标准 IEEE-802.3 CRC，初始种子为 $FFFFFFFF，最终需按位取反
  crc := not FCRC;
  isize := FSize;
  // little-endian
  tr[0] := Byte(crc and $FF);
  tr[1] := Byte((crc shr 8) and $FF);
  tr[2] := Byte((crc shr 16) and $FF);
  tr[3] := Byte((crc shr 24) and $FF);
  tr[4] := Byte(isize and $FF);
  tr[5] := Byte((isize shr 8) and $FF);
  tr[6] := Byte((isize shr 16) and $FF);
  tr[7] := Byte((isize shr 24) and $FF);
  FDest.WriteBuffer(tr, SizeOf(tr));
end;
...
begin
  ...
  if r <> SizeOf(hdr) then raise EArchiverError.Create('gzip: short header');
  if (hdr[0] <> $1F) or (hdr[1] <> $8B) or (hdr[2] <> 8) then
    raise EArchiverError.Create('gzip: invalid header');
  flg := hdr[3];
  // 保守校验：保留位必须为 0
  if (flg and $E0) <> 0 then
    raise EArchiverError.Create('gzip: invalid flags');
  ...
  if (flg and $04) <> 0 then begin
    ...
    if r <> SizeOf(len2) then raise EArchiverError.Create('gzip: short extra length');
    ...
    while toSkip > 0 do begin
      ...
      if r <> got then raise EArchiverError.Create('gzip: short extra');
  ...
  if (flg and $08) <> 0 then begin
    repeat
      r := FSource.Read(b, 1);
      if r <> 1 then raise EArchiverError.Create('gzip: short fname');
  ...
  if (flg and $10) <> 0 then begin
    repeat
      r := FSource.Read(b, 1);
      if r <> 1 then raise EArchiverError.Create('gzip: short fcomment');
      hdr_crc := _crc32_update(hdr_crc, b, 1);
    until b = 0;
  end;
  ...
  if (flg and $02) <> 0 then begin
    ...
    if r <> 2 then raise EArchiverError.Create('gzip: short fhcrc');
    got16 := crc16_le[0] or (crc16_le[1] shl 8);
    expect16 := Word((not hdr_crc) and $FFFF);
    if got16 <> expect16 then raise EArchiverError.Create('gzip: header crc mismatch');
  end;
...
var tr: array[0..7] of Byte; crc, isize: DWord; got, r: Longint; cur, total: Int64; need: Integer; p: Integer;
begin
  ...
  if _try_get_pos_size(FSource, cur, total) then begin
    if total < 8 then raise EArchiverError.Create('gzip: short trailer');
    if cur <> (total - 8) then
  ...
  while got < SizeOf(tr) do begin
    ...
    if r <= 0 then raise EArchiverError.Create('gzip: short trailer');
    Inc(got, r);
  end;
  // 如果此前用了前缀，清空以避免复用
  FTrailerPrefixLen := 0;
  ...
  if crc <> DWord(not FCRC) then raise EArchiverError.Create('gzip: crc mismatch');
  if isize <> FSize then raise EArchiverError.Create('gzip: size mismatch');
  FVerified := True;
end;
