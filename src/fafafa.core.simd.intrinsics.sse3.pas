...

{
  ...
  SSE3 (Streaming SIMD Extensions 3) 指令集支持
  
  SSE3 是 Intel 在 2004 年引入的 SIMD 指令集扩展
  主要增加了水平运算、复数运算和一些特殊的加载指令
...
function sse3_movddup_pd(const a: TM128): TM128;   // 复制双精度元素
...
begin
  // MONITOR 指令的占位符实现
  // 在实际实现中，这里应该执行 MONITOR 指令
end;

procedure sse3_mwait(extensions, hints: Cardinal);
begin
  // MWAIT 指令的占位符实现
  // 在实际实现中，这里应该执行 MWAIT 指令
end;
