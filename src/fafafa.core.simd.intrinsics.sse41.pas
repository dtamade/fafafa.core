...

{
  ...
  SSE4.1 (Streaming SIMD Extensions 4.1) 指令集支持
  
  SSE4.1 是 Intel 在 2006 年引入的 SIMD 指令集扩展
  主要增加了更多的整数运算、混合操作和字符串处理指令
...
// === SSE4.1 扩展的 Min/Max 操作 ===
function sse41_max_epi8(const a, b: TM128): TM128;   // 有符号8位最大值
function sse41_max_epi32(const a, b: TM128): TM128;  // 有符号32位最大值
function sse41_max_epu16(const a, b: TM128): TM128;  // 无符号16位最大值
function sse41_max_epu32(const a, b: TM128): TM128;  // 无符号32位最大值
function sse41_min_epi8(const a, b: TM128): TM128;   // 有符号8位最小值
function sse41_min_epi32(const a, b: TM128): TM128;  // 有符号32位最小值
function sse41_min_epu16(const a, b: TM128): TM128;  // 无符号16位最小值
function sse41_min_epu32(const a, b: TM128): TM128;  // 无符号32位最小值

// === SSE4.1 点积指令 ===
function sse41_dp_ps(const a, b: TM128; imm8: Byte): TM128;  // 单精度点积
function sse41_dp_pd(const a, b: TM128; imm8: Byte): TM128;  // 双精度点积

// === SSE4.1 混合操作 ===
function sse41_blend_ps(const a, b: TM128; imm8: Byte): TM128;     // 单精度混合
function sse41_blend_pd(const a, b: TM128; imm8: Byte): TM128;     // 双精度混合
function sse41_blendv_ps(const a, b, mask: TM128): TM128;          // 变量单精度混合
function sse41_blendv_pd(const a, b, mask: TM128): TM128;          // 变量双精度混合
function sse41_blendv_epi8(const a, b, mask: TM128): TM128;        // 变量8位整数混合

// === SSE4.1 舍入指令 ===
function sse41_round_ps(const a: TM128; rounding: Byte): TM128;    // 单精度舍入
function sse41_round_pd(const a: TM128; rounding: Byte): TM128;    // 双精度舍入
function sse41_round_ss(const a, b: TM128; rounding: Byte): TM128; // 标量单精度舍入
function sse41_round_sd(const a, b: TM128; rounding: Byte): TM128; // 标量双精度舍入

// === SSE4.1 插入/提取指令增强 ===
function sse41_insert_ps(const a, b: TM128; imm8: Byte): TM128;    // 插入单精度
function sse41_extract_ps(const a: TM128; imm8: Byte): Cardinal;   // 提取单精度
function sse41_insert_epi8(const a: TM128; Value: Integer; imm8: Byte): TM128;  // 插入8位整数
function sse41_insert_epi32(const a: TM128; Value: Integer; imm8: Byte): TM128; // 插入32位整数
function sse41_insert_epi64(const a: TM128; Value: Int64; imm8: Byte): TM128;   // 插入64位整数
function sse41_extract_epi8(const a: TM128; imm8: Byte): Integer;  // 提取8位整数
function sse41_extract_epi32(const a: TM128; imm8: Byte): Integer; // 提取32位整数
function sse41_extract_epi64(const a: TM128; imm8: Byte): Int64;   // 提取64位整数

// === SSE4.1 零扩展加载 ===
function sse41_loadl_epi64(const Ptr: Pointer): TM128;             // 加载64位并零扩展

// === SSE4.1 转换指令 ===
function sse41_cvtepi8_epi16(const a: TM128): TM128;   // 8位到16位符号扩展
function sse41_cvtepi8_epi32(const a: TM128): TM128;   // 8位到32位符号扩展
function sse41_cvtepi8_epi64(const a: TM128): TM128;   // 8位到64位符号扩展
function sse41_cvtepi16_epi32(const a: TM128): TM128;  // 16位到32位符号扩展
function sse41_cvtepi16_epi64(const a: TM128): TM128;  // 16位到64位符号扩展
function sse41_cvtepi32_epi64(const a: TM128): TM128;  // 32位到64位符号扩展
...
// === SSE4.1 其他指令 ===
function sse41_mullo_epi32(const a, b: TM128): TM128;              // 32位乘法低位
function sse41_mul_epi32(const a, b: TM128): TM128;                // 32位乘法到64位
function sse41_packus_epi32(const a, b: TM128): TM128;             // 32位打包到16位无符号饱和
