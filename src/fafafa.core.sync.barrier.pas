...
📖 概述：
  现代化、跨平台的 FreePascal 屏障同步实现，提供统一的 API 接口。
...
⚠️  重要说明：
  屏障适用于多线程同步场景，所有参与线程必须调用 Wait 方法。
  请根据具体场景选择合适的参与者数量。
 ...
 *
 * @param AParticipantCount 参与线程数量，必须 > 0
 * @return 屏障接口实例
 *
 * @desc
 *   创建一个屏障同步实例，自动选择当前平台的最优实现：
 *   - Windows: 基于 SynchronizationBarrier API 或 fallback 实现
 *   - Unix: 基于 pthread_barrier_t 或 fallback 实现
 *
 * @thread_safety
