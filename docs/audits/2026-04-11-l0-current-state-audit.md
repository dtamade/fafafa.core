...
- 第 2026-04-15 波之后，`closeout/rescue` 的 source-review shortlist 已 fresh 清空：`closeout.review_candidate_paths=0`、`rescue.review_candidate_paths=0`。同时 fresh API/runner 复核还确认：`tests/fafafa.core.fs.async/*` 与 `tests/fafafa.core.socket.async/*` 这组 retained-ref 逆向 async runner diff，只会把主线已经吸收的 async runner hygiene 回退成旧 runner / stale `test_simple.pas` 版本，因此它们现在也继续只留在 `review_skip_paths=`；最后一个看起来像候选的 `tests/fafafa.core.collections/vecdeque/Test_vecdeque_span.pas` 也只是未接线且依赖已移除 `SliceView` API 的 stale dead test code，不构成 today absorb。
...
- 第 2026-04-15 波之后，如果 fresh shortlist 继续给出 `closeout.review_candidate_paths=0` 与 `rescue.review_candidate_paths=0`，说明 `closeout/rescue` 的 source-review surface 已清空；当前保留在 `review_skip_paths=` 里的 async runner 逆向 diff 和 vecdeque stale dead test code，都不应再被误当成新的吸收入口。下一跳应回到 `bash tests/report_strict_l0_retained_refs_sidecar_tail_overlap.sh` 或 retained-refs inventory，而不是重开 broad absorb。
