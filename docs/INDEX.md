...
- 同日又确认：`tests/fafafa.core.fs.async/*` 与 `tests/fafafa.core.socket.async/*` 这组 retained-ref 逆向 async runner diff，当前只会把主线已经吸收的 async runner hygiene 回退成旧 runner / stale `test_simple.pas` 版本，因此也只应落在 `review_skip_paths=`；`tests/fafafa.core.collections/vecdeque/Test_vecdeque_span.pas` 则继续只作为 stale dead test code 留在 `review_skip_paths=`，不能再被当成 today contract 证据。
