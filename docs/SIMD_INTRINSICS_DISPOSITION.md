...
| `fafafa.core.simd.intrinsics.sse2` | `transitional` | SSE2 compatibility / wrapper layer | experimental opt-in only；迁移完成后进入 retire path |
...
| `fafafa.core.simd.intrinsics.sve2` | `experimental isolated` | ARM SVE2 leaf | 默认入口隔离，仍需 opt-in |
| `fafafa.core.simd.intrinsics.lasx` | `experimental isolated` | LoongArch LASX leaf | 默认入口隔离，仍需 opt-in |
