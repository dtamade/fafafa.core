...
- `check` 现在还包含 `dispatch-read-scope`、`dataplane-consumer-scope` 与 `metadata-query-scope`：`dispatch-read-scope` 把 `GetDispatchTable` 的直接读取限制在 `dispatch` / `dataplane` / `runtime` 这三处内部单元；`dataplane-consumer-scope` 把 `GetCurrentSimdDataPlane` / `GetCurrentSimdDataPlaneDispatch` / `RebindSimdDataPlane` 的直接消费限制在 `dataplane` / `direct` / `public ABI` / façade companion surfaces，防止其他模块长出第二条 publication-consumer path；`metadata-query-scope` 则把 `GetBackendInfo` / `TryGetRegisteredBackendDispatchTable` / backend text getters 的直接使用限制在 `dispatch` / `runtime` / `public ABI` / `backend adapter` 内部，防止 façade / companion surfaces 又长出第二条 metadata truth path。
