...
- `check` 现在还包含 `dispatch-read-scope` 与 `metadata-query-scope`：前者把 `GetDispatchTable` 的直接读取限制在 `dispatch` / `dataplane` / `runtime` 这三处内部单元；后者把 `GetBackendInfo` / `TryGetRegisteredBackendDispatchTable` / backend text getters 的直接使用限制在 `dispatch` / `runtime` / `public ABI` / `backend adapter` 内部，防止 façade / companion surfaces 又长出第二条 metadata truth path。
