...
| `SVE2`    | no stable adapter target          | `src/fafafa.core.simd.intrinsics.sve2.pas`                                                 | `experimental isolated`                  | `experimental-intrinsics` isolation, `SVE2` non-qualified-host runtime reject smoke                                                                                        | no             | no                               | Wave 4 hold          | 继续 hold；当前只按 base-`SVE` 资格做 runtime fail-close，不把它误写成 stable `SVE2` contract；reopen 触发条件看对应 hold plan |
| `LASX`    | no stable adapter target          | `src/fafafa.core.simd.intrinsics.lasx.pas`                                                 | `experimental isolated`                  | `experimental-intrinsics` isolation, `LASX` non-qualified-host runtime reject smoke                                                                                        | no             | no                               | Wave 4 hold          | 继续 hold；当前先守住非 `LoongArch64` runtime fail-close，feature-level qualification 仍等待 `cpuinfo`/evidence 路线 |
