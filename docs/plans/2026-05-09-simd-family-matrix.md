...
| `SSE2`    | `src/fafafa.core.simd.sse2.pas`   | `src/fafafa.core.simd.intrinsics.sse2.pas`, `src/fafafa.core.simd.intrinsics.x86.sse2.pas` | `transitional` + `active leaf` | `sse2-structure-check`, `impl-smoke-sse2`, `check`, `gate`, `test_sse2_raw_leaf_parity (90 checks)`                                                                         | yes            | yes                              | Wave 5 retire ready  | raw leaf 已 promote；下一步：stable adapter 开始委托 raw leaf，transitional wrapper 进入 retire path     |
...
| `NEON`    | `src/fafafa.core.simd.neon.pas`   | `src/fafafa.core.simd.intrinsics.neon.pas`                                                 | `experimental isolated`                  | `nonx86-optin-list-suites`, `check_nonx86_register_truthfulness.py --backend neon`, `impl-smoke-nonx86`, `impl-audit-nonx86`, `closeout-host-local`, `NEON` non-qualified-host runtime reject smoke | yes            | adapter yes, leaf no             | Wave 4 qualification | 先把 adapter truth / leaf status / non-x86 evidence 串成可复验 lane；leaf runtime 现在也只在 `cpuinfo` 报告 `NEON` 的目标主机放行 |
| `RISCVV`  | `src/fafafa.core.simd.riscvv.pas` | `src/fafafa.core.simd.intrinsics.rvv.pas`                                                  | `experimental isolated`                  | `nonx86-optin-list-suites`, `check_nonx86_register_truthfulness.py --backend riscvv`, `impl-smoke-nonx86`, `impl-audit-nonx86`, `riscvv-opcode-lane`, `closeout-host-local`, `RVV` non-qualified-host runtime reject smoke | opt-in only    | no default gate as stable family | Wave 4 qualification | 继续保持 opt-in；先做 qualification，不急着进入 default stable 路径；leaf runtime 现在也只在 `cpuinfo` 报告 `RVV` 的目标主机放行 |
| `AES`     | no stable adapter target          | `src/fafafa.core.simd.intrinsics.aes.pas`                                                  | `experimental isolated`                  | `experimental-intrinsics` isolation, `Test_Default_AES_SHA_Rejects`, `Test_Experimental_AES_SHA_PlaceholderSemantics`，以及 cross-host opt-in contract note                | no             | no                               | Wave 4 hold          | 继续 hold；当前实验 lane 只锁 default-reject + placeholder semantics，并明确不把 non-x86 runtime reject 写成现合同；具体 reopen 触发条件看 `simd-experimental-hold-future-trigger-plan` 的 `AES` 行 |
| `SHA`     | no stable adapter target          | `src/fafafa.core.simd.intrinsics.sha.pas`                                                  | `experimental isolated`                  | `experimental-intrinsics` isolation, `Test_Default_AES_SHA_Rejects`, `Test_Experimental_AES_SHA_PlaceholderSemantics`，以及 cross-host opt-in contract note                | no             | no                               | Wave 4 hold          | 继续 hold；当前实验 lane 只锁 default-reject + placeholder semantics，并明确不把 non-x86 runtime reject 写成现合同；具体 reopen 触发条件看 `simd-experimental-hold-future-trigger-plan` 的 `SHA` 行 |
...
| `SVE`     | no stable adapter target          | `src/fafafa.core.simd.intrinsics.sve.pas`                                                  | `experimental isolated`                  | `experimental-intrinsics` isolation, `SVE` non-qualified-host runtime reject smoke                                                                                         | no             | no                               | Wave 4 hold          | 继续 hold；当前 experimental runtime 只在 `cpuinfo` 报告 `SVE` 的 `AArch64` 主机放行；reopen 触发条件看 `simd-experimental-hold-future-trigger-plan` 的 `SVE` 行 |
| `SVE2`    | no stable adapter target          | `src/fafafa.core.simd.intrinsics.sve2.pas`                                                 | `experimental isolated`                  | `experimental-intrinsics` isolation, `SVE2` non-qualified-host runtime reject smoke                                                                                        | no             | no                               | Wave 4 hold          | 继续 hold；当前已按 `SVE2` 资格做 runtime fail-close，不把它误写成 stable `SVE2` contract；reopen 触发条件看对应 hold plan |
| `LASX`    | no stable adapter target          | `src/fafafa.core.simd.intrinsics.lasx.pas`                                                 | `experimental isolated`                  | `experimental-intrinsics` isolation, `LASX` non-qualified-host runtime reject smoke                                                                                        | no             | no                               | Wave 4 hold          | 继续 hold；当前 experimental runtime 只在 `cpuinfo` 报告 `LASX` 的 `LoongArch64` 主机放行；reopen 触发条件仍看 hold plan |
