...
`fafafa.core.mem` 当前负责 L0 里的内存操作、分配器合同，以及几类可直接复用的池实现。这个根文档只负责定锚 source-of-truth、模块边界和阅读顺序，不再继续承担“第二套大全手册”。
...
7. `src/fafafa.core.mem.interfaces.pas`
8. `src/fafafa.core.mem.stats.pas`
9. `tests/fafafa.core.mem.allocator.foundation/README.md`
10. `tests/fafafa.core.mem/README.md`
11. `examples/fafafa.core.mem/README.md`
12. `docs/mem/README.md`
...
- `fafafa.core.mem.allocator.foundation` 是 strict L0 allocator contract 的推荐入口。
...
- strict L0 allocator 回归入口现在单独放在 `tests/fafafa.core.mem.allocator.foundation/`，不再由 `tests/fafafa.core.mem/` 单独承担。
