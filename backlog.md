...
- [ ] **P1 / repo**：梳理并最小化“运行产物进入版本库”的风险（例如已跟踪的 logs/reports）  
  进度：已完成十二批运行产物清理，累计去追踪 **631** 项（`tests/` 97 + `examples/` 147 + `bin/` 2 + `reference/lockfree-develop` 53 + `reference/xxHash-dev` 89 + `reference/DCPcrypt` 92 + `reference/tomlc99-master` 151），并补充 `.gitignore` 防回潮；最近四批已摘掉 `tests/fafafa.core.sync.barrier/` 下 6 个已跟踪运行输出、`tests/fafafa.core.atomic/` 与 `tests/fafafa.core.mem.manager.rtl/` 下 3 个已跟踪生成物、`tests/fafafa.core.archiver/` / `tests/fafafa.core.env/` 下 3 个历史运行日志，以及 `tests/fafafa.core.fs/performance-data/` 下 18 个 latest/history 性能结果文件（保留 baseline 类基线文件）。本轮另已把 `docs/collections/reports/` 与 `docs/benchmarks/reports/` 下 17 份阶段性 campaign 报告归档到 `archive/reports/`，原目录只保留指路 README；并把 `docs/reports/` 根下 13 份无活引用或仅历史互引的 dated fix/checkpoint/verification/audit 报告迁到 `archive/reports/docs-root/`，同时把 `UnChecked_Methods_Summary.md` 转正到 `docs/collections/guides/`；另外又把 6 份 root-level 的模块完成/测试日报迁到 `archive/reports/docs-root/`。最新几批继续把 `docs/` 根下 14 份 non-SIMD 历史状态/总结/实施文档、2 份 `benchmark v2` 历史完成总结迁到 `archive/reports/docs-root/`，并把 `MUTEX_IMPLEMENTATION.md` 从根层归位到 `docs/topics/sync/mutex/mutex_implementation.md`；同时修正 `docs/INDEX.md` 与 mem current-entry 中对 `docs/mem/`、`docs/term/`、`docs/fs/`、`docs/lockfree/`、`docs/simd/` 的假导航表述。最新四批又修掉了 `tests/fafafa.core.fs/` 下 4 个 perf shell runner、`tests/` 侧 16 个活跃 shell runner，以及 `examples/` / `benchmarks/` 侧 6 个活跃 shell runner 的 CRLF/LF 与默认 build mode 合同问题，新增 `tests/test_fs_perf_shell_scripts.sh`、`tests/test_active_shell_runners.sh`、`tests/test_socket_smoke_runner.sh`、`tests/test_example_benchmark_shell_runners.sh` 四条回归守卫，并把这些检查统一接进 `tests/test_repo_hygiene_guard.sh`；`examples/fafafa.core.json` 这一组也已从“runner/工程漂移”收口到真实可用：`BuildOrRun.sh`、`BuildOrRun_Min.sh`、`BuildOrRun_NoExcept.sh`、`BuildOrRun_NoExcept_Writer.sh` 在当前 Linux 沙箱均可 fresh build+run，通过点包括 `example_json.lpr` 的迭代 API 对齐，以及 `fafafa.core.json.interfaces` 对 UTF-8 helper 与 read/write flag 常量的 alias 层补齐。

## Next（下一步）
- [ ] **P1 / repo**：沿 `examples/` / `benchmarks/` 继续扫描 non-SIMD 活跃 shell runner，按“LF/语法/默认 build mode 合同”拆下一批 hygiene
- [ ] **P1 / examples**：继续扫 `examples/` 下 non-SIMD 工程的 alias/runner 漂移，优先挑“可小步闭环”的 JSON sibling 或 process/env 旁支
...
## Done（已完成）
- [x] 2026-04-08：`examples/fafafa.core.json` 的 source-level blocker 已收口：`example_json.lpr` 已对齐当前 iterator/allocator API，`fafafa.core.json.interfaces` 已去掉不合法的 `export` 并补齐 flag 常量 alias，`BuildOrRun.sh` / `BuildOrRun_NoExcept.sh` / `BuildOrRun_NoExcept_Writer.sh` 均已 fresh PASS
- [x] 2026-04-08：`examples/` / `benchmarks/` 侧 6 个活跃 shell runner 已统一为 LF，并补 `tests/test_example_benchmark_shell_runners.sh` 守卫；`examples/fafafa.core.env/BuildOrRun.sh` 的默认 build mode / `set -e` 误退问题与 `examples/fafafa.core.json` 最小示例的 Linux `.lpi` 路径问题已收口
- [x] 2026-04-08：`tests/` 侧 16 个活跃 shell runner 已统一为 LF，新增 `tests/test_active_shell_runners.sh` / `tests/test_socket_smoke_runner.sh` 守卫，并修复 `tests/fafafa.core.socket/smoke.sh` 的默认 build mode、`for` 语法与退出码处理
- [x] 2026-04-08：`tests/fafafa.core.fs/` 的 4 个 perf shell runner 已统一为 LF，并新增 `tests/test_fs_perf_shell_scripts.sh` 防止 CRLF/bash 语法回潮
- [x] 2026-04-08：`MUTEX_IMPLEMENTATION.md` 已从根层迁到 `docs/topics/sync/mutex/mutex_implementation.md`，sync.mutex 专题入口与 example 引用已同步
- [x] 2026-04-08：`benchmark v2` 两份历史完成总结已迁到 `archive/reports/docs-root/`，根层不再让 `final` / `final.complete` 与当前 benchmark 入口竞争
