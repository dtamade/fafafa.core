...
begin
  FBuffer.Free;
  // 只有当FGrowStrategy不是全局单例时才释放
  if FGrowStrategy <> GetDefaultGrowStrategy then
    FGrowStrategy.Free;
  inherited Destroy;
end;
...
begin
  ...
  // 检查是否是全范围搜索
  isFullRange := (aStartIndex = 0) and (aCount = GetCount);
...
begin
  ...
  LNewDeque := TVecDeque.Create(VECDEQUE_DEFAULT_CAPACITY, FBuffer.GetAllocator, FGrowStrategy, nil);
