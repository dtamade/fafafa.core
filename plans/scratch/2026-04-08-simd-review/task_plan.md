...
在 `movemask` 小簇收口后，继续沿 `sse2_min_refs=3` 的 scouting 口径挑最小 residual；这次锁定的是 `simd_cvtsi128_si32` 与 `simd_cvtsi128_si64`。目标仍然不是改 checker，而是在已有 conversion 语义测试里补第二组真实 extraction witness，把这 2 个名字从 `refs=2` 收上去。

### Phases

| Phase | Status | Notes |
| --- | --- | --- |
| 1. 用 `sse2_min_refs=3` 复核最小 residual 簇 | completed | 已确认 `cvtsi128_si32/si64` 是当前最小 remaining 小簇，且都已集中在同一个 conversion testcase 段里 |
| 2. 在现有 conversion 测试里补 repeated runtime witness | completed | 已在 `Test_ConversionFamilies_PreserveExpectedLanes` 增加第二组 low-dword/low-qword extraction witness，直接走不同 bit pattern 的 `TM128` 输入 |
| 3. 定向/主线验证与 scratch 收口 | completed | `git diff --check`、`sse2_min_refs=3` 定向统计、release `experimental-intrinsics-tests`、release `gate` 全部通过；本批仍只同步 scratch，不改 active maintenance 口径 |
...
| `DispatchAPI` 的 `WideFallbackOnly` 旧断言在 `AndNotU8x16` 翻案后首轮仍引用旧合同 | 1 | 删掉残留的 `AndNotU8x16` 旧断言，并把 dedicated truth 收敛到 `Test_RISCVV_AndNotSlots_Keep_AsmOwnedCompositions_And_Reuse_BaseScalar_When_NoAsm` 后，Release `DispatchAPI` 恢复 PASS |
| `check_interface_implementation_completeness.py` 首轮仍把 `DotF64x2/F64x4` 与 wide `Ceil/Floor/Round/Trunc` 的 intentional base-scalar reuse 误报成 P1/P2 缺口 | 1 | 新增 `RISCVV_DOT_BASE_SCALAR_SLOTS` 与 `RISCVV_WIDE_ROUNDING_BASE_SCALAR_SLOTS`，strict completeness 重新回到 `P0/P1/P2 = 0` |
| 首轮把 `check_interface_implementation_completeness.py` 当成支持 `--summary-line` 的 checker | 1 | 现场执行后确认 CLI 只支持 `--strict/--strict-level`；已更正文档命令并用正确参数复核 `P0/P1/P2 = 0` |
...
把 `RISCVV MinF32x8/MaxF32x8/MinF32x16/MaxF32x16` 这组紧邻 residual 从“缺 guard 的可疑点”收口成可提交结论：先用 `NonX86IEEE754` special-case parity 证明当前 host 上是否真有 `NaN / signed-zero` 漂移；如果没有，就保持实现不动，只补齐 source/runtime 双护栏。
...
复核 `RISCVV` wide `Ceil/Floor/Round/Trunc` 与 `ClampF32` 这 18 个 slot，确认它们当前是否真的还需要发布 RISCVV family-local scalar-forward wrapper；如果 asm/no-asm 两侧都只是把调用再转给 `Scalar*`，就把这整簇收成 `reuse_base_scalar`，只保留真正 still-backend-owned 的 `ClampF64x4/F64x8`。

### Phases

| Phase | Status | Notes |
| --- | --- | --- |
| 1. 重新对位 source/register/runtime split | completed | 已确认 `src/fafafa.core.simd.riscvv.register.inc` 之前仍给 `Ceil/Floor/Round/TruncF32x8/F64x4/F32x16/F64x8` 与 `ClampF32x8/F32x16` 发布 backend binding；但 `src/fafafa.core.simd.riscvv.pas` 的 `{$IFDEF RISCVV_ASSEMBLY}` 同名函数也只是 exact `Scalar*` forward，`src/fafafa.core.simd.riscvv.facade.inc` 的 no-asm body 同样只是 exact `Scalar*` forward。因此这批不是“backend-owned local semantics”，而是整格 over-retained scalar-forward wrapper |
| 2. 收源码与 checker 护栏 | completed | `src/fafafa.core.simd.riscvv.register.inc` 已删除这 18 个 slot 的 register binding；`src/fafafa.core.simd.riscvv.pas` 已删除 wide `Ceil/Floor/Round/Trunc` 与 `ClampF32` 的 asm-side dead scalar-forward wrapper；`src/fafafa.core.simd.riscvv.facade.inc` 已删除对应 no-asm dead wrapper；`tests/fafafa.core.simd/check_nonx86_register_truthfulness.py` / `check_nonx86_key_slot_audit.py` / `check_nonx86_helper_semantics.py` 与 `tests/fafafa.core.simd/fafafa.core.simd.dispatchapi.testcase.pas` 已同步翻正为 `Test_RISCVV_WideRoundingAndF32ClampSlots_Reuse_BaseScalar_When_ScalarForwarders_Are_Dead` 这套合同 |
| 3. 串行 Release 复验并确认 stop-point | completed | `git diff --check`、`python3 -m py_compile tests/fafafa.core.simd/check_nonx86_helper_semantics.py tests/fafafa.core.simd/check_nonx86_key_slot_audit.py tests/fafafa.core.simd/check_nonx86_register_truthfulness.py`、`python3 tests/fafafa.core.simd/check_nonx86_helper_semantics.py --summary-line`、`python3 tests/fafafa.core.simd/check_nonx86_register_truthfulness.py --backend riscvv --summary-line --strict`、`python3 tests/fafafa.core.simd/check_nonx86_key_slot_audit.py --backend riscvv --summary-line`、Release `TTestCase_DispatchAPI,TTestCase_NonX86BackendParity`、Release `impl-audit-nonx86`、Release `check` 全部 fresh 通过；关键结果包括 `checks=740`、`backend=riscvv assignments=443 wrapper_only=14 no-asm wrapper ok=0`、`NONX86_KEY_SLOT_AUDIT_SUMMARY backends=neon,riscvv slots=136 issues=0 status=ok`、`NONX86_IMPL_AUDIT_SUMMARY ... status=ok` |

## 2026-05-18 RISCVV DotF64 Scalar-Forward Slot Closeout

### Goal

把 `RISCVV DotF64x2/DotF64x4` 这最后一对看起来像 backend-owned、实际只是 dead scalar-forward wrapper 的 residual 收口成真实合同：删除 source/facade dead owner，撤掉 register binding，并把 runtime slot 翻正为 `reuse_base_scalar`。

### Phases

| Phase | Status | Notes |
| --- | --- | --- |
| 1. 重新核对旧结论是不是已经被 fresh evidence 推翻 | completed | 已确认 `src/fafafa.core.simd.riscvv.register.inc` 之前仍绑定 `DotF64x2/F64x4`，但 `src/fafafa.core.simd.riscvv.pas` 与 `src/fafafa.core.simd.riscvv.facade.inc` 的同名函数都只是 exact `ScalarDotF64x2/F64x4` forward；结合 fresh direct registered-table parity 没有打出 drift，旧 scratch 中“DotF64 必须 backend-owned”的结论已被翻正 |
| 2. 收源码与 checker 护栏 | completed | `src/fafafa.core.simd.riscvv.register.inc` 已删除 `DotF64x2/F64x4` binding；`src/fafafa.core.simd.riscvv.pas` 与 `src/fafafa.core.simd.riscvv.facade.inc` 已删除 dead `RISCVVDotF64x2/F64x4` wrapper；`tests/fafafa.core.simd/check_nonx86_helper_semantics.py` 已改成 absent-routine truth；`check_nonx86_register_truthfulness.py` 与 `check_nonx86_key_slot_audit.py` 已把这两个 slot 从 backend-owned wrapper 账本切回 `reuse_base_scalar`；`tests/fafafa.core.simd/fafafa.core.simd.dispatchapi.testcase.pas` 已新增 `Test_RISCVV_DotF64Slots_Reuse_BaseScalar_When_ScalarForwarders_Are_Dead` |
| 3. 串行 Release 复验并确认 stop-point | completed | `git diff --check`、`python3 -m py_compile tests/fafafa.core.simd/check_nonx86_helper_semantics.py tests/fafafa.core.simd/check_nonx86_key_slot_audit.py tests/fafafa.core.simd/check_nonx86_register_truthfulness.py`、`python3 tests/fafafa.core.simd/check_nonx86_helper_semantics.py --summary-line`、`python3 tests/fafafa.core.simd/check_nonx86_register_truthfulness.py --backend riscvv --summary-line --strict`、`python3 tests/fafafa.core.simd/check_nonx86_key_slot_audit.py --backend riscvv --summary-line`、Release `TTestCase_DispatchAPI,TTestCase_NonX86BackendParity`、Release `impl-audit-nonx86`、Release `check` 全部 fresh 通过；关键结果包括 `checks=740`、`backend=riscvv assignments=441 wrapper_only=12 no-asm wrapper ok=0`、`NONX86_KEY_SLOT_AUDIT_SUMMARY backends=neon,riscvv slots=136 issues=0 status=ok`、`NONX86_IMPL_AUDIT_SUMMARY ... status=ok` |

## 2026-05-18 RISCVV Exact Scalar Helper Slot Closeout

### Goal

把 `RISCVV` 剩余 `wrapper_only=12` 里那 9 个在 asm/common 与 no-asm 两边都只是 exact `Scalar*` forward 的 `I64/U64/Cmp/Min/Max` 槽位收口成 `reuse_base_scalar`，只把真正仍有局部组合语义的 `AndNotI8x16/U16x8/U8x16` 留在 backend-owned residual。

### Phases

| Phase | Status | Notes |
| --- | --- | --- |
| 1. 分组 residual 并确认这 9 个 slot 没有真实 backend-local 语义 | completed | 已确认 `src/fafafa.core.simd.riscvv.pas` 与 `src/fafafa.core.simd.riscvv.helpers.inc` 的 `AndNotI64x2/MinI64x2/MaxI64x2/AndNotU64x2/CmpEqU64x2/CmpLtU64x2/CmpGtU64x2/MinU64x2/MaxU64x2` 两边都只是 exact `Scalar*` forward；不同于 `AndNotI8x16/U16x8/U8x16` 这 3 个仍保留 local composition，因此这 9 个是真正的 over-retained owner |
| 2. 收源码与 checker 护栏 | completed | `src/fafafa.core.simd.riscvv.pas` 与 `src/fafafa.core.simd.riscvv.helpers.inc` 已删除这 9 个 dead owner；`src/fafafa.core.simd.riscvv.register.inc` 已删除对应 binding；`tests/fafafa.core.simd/check_nonx86_helper_semantics.py` 已改成 source/helpers absent-routine truth；`check_nonx86_register_truthfulness.py` 只剩 3 个 `AndNot*` allowlist；`check_nonx86_key_slot_audit.py` 已新增 `RISCVV_EXACT_SCALAR_REUSE_KEY_SLOTS` 与 `Test_RISCVV_ExactScalarHelperSlots_Reuse_BaseScalar_When_Owners_Are_Dead`；旧 `Test_RISCVV_HelperOwnedExactScalarSlots_Stay_BackendOwned` 已收窄回真正仍然 backend-owned 的 3 个 `AndNot*` 组合槽位 |
| 3. 串行 Release 复验并确认 stop-point | completed | `git diff --check`、`python3 -m py_compile tests/fafafa.core.simd/check_nonx86_helper_semantics.py tests/fafafa.core.simd/check_nonx86_key_slot_audit.py tests/fafafa.core.simd/check_nonx86_register_truthfulness.py`、`python3 tests/fafafa.core.simd/check_nonx86_helper_semantics.py --summary-line`、`python3 tests/fafafa.core.simd/check_nonx86_register_truthfulness.py --backend riscvv --summary-line --strict`、`python3 tests/fafafa.core.simd/check_nonx86_key_slot_audit.py --backend riscvv --summary-line`、Release `TTestCase_DispatchAPI,TTestCase_NonX86BackendParity`、Release `impl-audit-nonx86`、Release `check` 全部 fresh 通过；关键结果包括 `checks=740`、`backend=riscvv assignments=432 wrapper_only=3 no-asm wrapper ok=0`、`NONX86_KEY_SLOT_AUDIT_SUMMARY backends=neon,riscvv slots=136 issues=0 status=ok`、`NONX86_IMPL_AUDIT_SUMMARY ... status=ok` |

## 2026-05-18 RISCVV AndNot Asm-Only Composition Contract Closeout

### Goal

把最后 3 个 `wrapper_only` 槽位 `AndNotI8x16/U16x8/U8x16` 从“看起来像 dead scalar-forward residual”翻正成真实合同：`asm` 打开时保留 `RISCVV` 本地组合实现与 register ownership；`no-asm` host 不再伪造 helper-owned backend slot，而是直接 reuse `FillBaseDispatchTable` 的 scalar slot。

### Phases

| Phase | Status | Notes |
| --- | --- | --- |
| 1. 重新核对这 3 个 `AndNot*` 到底是不是 dead owner | completed | 已确认 `src/fafafa.core.simd.riscvv.pas` 里的 `RISCVVAndNotI8x16/U16x8/U8x16` 不是 exact `Scalar*` forward，而是仍保留 `RISCVVNot* + RISCVVAnd*` 的 local composition；因此它们与刚删掉的 `I64/U64/Cmp/Min/Max` dead owner 不同，不能继续按 `always wrapper-owned` 或 `reuse_base_scalar` 一刀切 |
| 2. 收正源码、checker 与 dedicated truth | completed | `src/fafafa.core.simd.riscvv.pas` 里这 3 个 wrapper 已收窄到 `{$IFDEF RISCVV_ASSEMBLY}`；`src/fafafa.core.simd.riscvv.helpers.inc` 已删除对应 no-asm `ScalarAndNot*` helper owner；`src/fafafa.core.simd.riscvv.register.inc` 已改成 asm-only binding；`check_nonx86_helper_semantics.py` 改为要求 `riscvv.pas` 保留 local composition、`riscvv.helpers.inc` 不得再保留这 3 个 routine；`check_nonx86_register_truthfulness.py` 已把这 3 个翻正为合法 `asm-only wrapper`；`check_nonx86_key_slot_audit.py` 与 `DispatchAPI` 统一改成 `Test_RISCVV_AndNotSlots_Keep_AsmOwnedCompositions_And_Reuse_BaseScalar_When_NoAsm` |
| 3. 串行 Release 复验并确认 stop-point | completed | `git diff --check`、`python3 -m py_compile tests/fafafa.core.simd/check_nonx86_helper_semantics.py tests/fafafa.core.simd/check_nonx86_key_slot_audit.py tests/fafafa.core.simd/check_nonx86_register_truthfulness.py tests/fafafa.core.simd/check_interface_implementation_completeness.py`、`python3 tests/fafafa.core.simd/check_nonx86_helper_semantics.py --summary-line`、`python3 tests/fafafa.core.simd/check_nonx86_register_truthfulness.py --backend riscvv --summary-line --strict`、`python3 tests/fafafa.core.simd/check_nonx86_key_slot_audit.py --backend riscvv --summary-line`、`python3 tests/fafafa.core.simd/check_interface_implementation_completeness.py --strict --strict-level p2`、Release `TTestCase_DispatchAPI,TTestCase_NonX86BackendParity`、Release `impl-audit-nonx86`、Release `check`、Release `gate` 全部 fresh 通过；关键结果包括 `NONX86_HELPER_SEMANTICS_SUMMARY checks=743 status=ok`、`NONX86_REGISTER_TRUTHFULNESS_SUMMARY backend=riscvv assignments=432 asm_exact=312 asm_suffix_only=117 wrapper_only=3 scalar_passthrough=0 no_def=0 miswired=0 unused_allowlist=0 strict=1`、`asm-only wrapper ok=3`、`NONX86_KEY_SLOT_AUDIT_SUMMARY backends=riscvv slots=71 issues=0 status=ok`，以及 strict completeness `P0/P1/P2 = 0` |

## 2026-05-18 SSE2 Raw-Leaf Qualification Coverage Expansion

### Goal

在不重开 `SSE2` family-level 迁移讨论的前提下，补齐 `tests/fafafa.core.simd.intrinsics.experimental/` 这条 lane 里最薄的一层 raw semantic proof，让当前 `SSE2` retire baseline 至少不再缺 bitwise / lane-order / float arithmetic 的代表性 leaf 证据。
...
顺着刚在 `sqrt` 上证实过的特殊值 exception 泄漏模式，继续只切 `div` 小簇：给 `simd_div_ps`、`simd_div_pd`、`simd_div_sd` 补 `1/0`、`-1/0`、`0/0`、`qNaN/1` witness。若 fresh proof 真打红，就只把修复收敛在这 3 个 leaf，并保持 packed lane / scalar high-lane preserve 合同。
