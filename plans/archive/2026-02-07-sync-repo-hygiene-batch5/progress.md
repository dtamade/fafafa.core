# Progress Log: sync forced-named closure + repo hygiene batch5
...
- 统计 ignored-but-tracked 分布，确认存量主要集中在 `reference/`。
- 审计 `reference/` 子目录规模并选择最小可控批次 `reference/lockfree-develop`（53 项）。
- 验证 `reference/` 在 `src/tests/examples/docs` 中无直接引用后，执行 `reference/lockfree-develop` 去追踪。
...
| `git ls-files -ci --exclude-standard reference/lockfree-develop | wc -l` (before) | >0 | 53 | INFO |
| `git diff --cached --name-status -- reference/lockfree-develop | wc -l` | 53 | 53 | PASS |
| `git ls-files -ci --exclude-standard reference/lockfree-develop | wc -l` (after) | 0 | 0 | PASS |
| `git ls-files -ci --exclude-standard | wc -l` | 下降 | 3051 -> 2996 | PASS |

### Notes
- 本轮完成 `P0/sync` 的“强制 named 实测闭环”。
- 本轮完成 `P1/repo` 第四批（`bin/` 2 项）+ 第五批（`reference/lockfree-develop` 53 项）小步清理。
- 待做（交付前）：更新 `backlog.md` 的 Now/Done，并归档三文件到 `plans/archive/2026-02-07-sync-repo-hygiene-batch5/`。
