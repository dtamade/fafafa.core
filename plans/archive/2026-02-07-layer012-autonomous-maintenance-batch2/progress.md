...
### Batch2 Execution Log (2026-02-07)

| Command | Purpose | Result |
|---|---|---|
| `STOP_ON_FAIL=1 bash tests/run_all_tests.sh fafafa.core.process` | 复现 process 编译阻断 | 先后定位语法/标识符问题并修复 |
| `bash tests/fafafa.core.process/BuildOrTest.sh build` | 验证 process 编译可过 | PASS |
| `STOP_ON_FAIL=1 bash tests/run_all_tests.sh fafafa.core.socket` | 复现 socket 编译阻断 | 先后修复 include/语法/兼容问题后进入运行阶段 |
| `STOP_ON_FAIL=1 bash tests/run_all_tests.sh fafafa.core.toml` | 判定 toml 当前失败层级 | 运行断言失败（非编译） |
| `STOP_ON_FAIL=1 bash tests/run_all_tests.sh fafafa.core.xml` | 判定 xml 当前失败层级 | 运行断言失败（非编译） |

### Batch2 Outcome
- `process`：**编译阻断已清除**，测试进入运行阶段。
- `socket`：**编译阻断已清除**，测试进入运行阶段；当前沙箱因禁网导致 `EPERM` 运行失败。
- `toml/xml`：维持“断言失败”状态，待 batch3/4 做根因收敛。
