...
- [x] 更新 `backlog.md`（Now/Next/Done + batch2 指向）
- [x] 归档三文件到 `plans/archive/2026-02-07-layer012-autonomous-maintenance-batch1/`
- **Status:** complete
...
## Batch2 Update (2026-02-07)

### Phase 6: Batch2 Execution（process/socket 编译阻断）
- [x] 修复 `src/fafafa.core.process.unix.inc` 条件编译分支错位（`BEGIN expected but IF found`）
- [x] 修复 Unix 兼容错误码/错误消息：`StrError` -> `SysErrorMessage`，`ECHILD` 使用 `ESysECHILD` 兼容守卫
- [x] 修复 process 测试编译阻断（`fpSetEnv/fpUnsetEnv` 兼容、`CheckTrue` 参数顺序）
- [x] 修复 `src/fafafa.core.process.pas` 的跨平台链接守卫（`kernel32` 声明仅在 Windows）
- [x] 修复 `tests/fafafa.core.process/tests_process.lpr` 线程驱动缺失（Unix 加 `cthreads`）
- [x] 修复 `src/fafafa.core.socket.linux.inc` 的 include 语法阻断（移除内嵌 `uses`）
- [x] 补齐 Linux include 所需类型/符号定义与 `getaddrinfo/freeaddrinfo` 绑定
- [x] 清理 Linux include 中 FPC 3.3.1 不支持的行内 `var` 与 `MSG_NOSIGNAL` 常量声明写法
- [x] 修复 `src/fafafa.core.socket.pas` 超时函数中的行内 `var` 语法
- [x] 修复 `tests/fafafa.core.socket/tests_socket.lpr` 线程驱动缺失（Unix 加 `cthreads`）

### Verification Snapshot
- `process`：编译通过（`bash tests/fafafa.core.process/BuildOrTest.sh build`）。
- `socket`：编译通过，测试运行期在当前沙箱环境受限（禁网导致 socket 创建 `EPERM`）。
- `toml/xml`：仍为断言失败（非编译阻断）。

### Phase 7: Next Batch Proposal
- [ ] batch3：`toml` 公共根因优先（unicode/reader/error-prefix）
- [ ] batch4：`xml` 按 writer/reader/chunk-coalesce 分组收敛
- [ ] batch5：`socket` 受限环境口径（编译基线 vs 运行基线）
