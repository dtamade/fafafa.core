program fafafa.core.sync.barrier.benchmark;
...
type
  ...
  public
    constructor Create(ABarrier: IBarrier; AOperations: PInt64; ADurationNs: Int64; const AStartTime: THighResTime);
  end;

constructor TWorkerThread.Create(ABarrier: IBarrier; AOperations: PInt64; ADurationNs: Int64; const AStartTime: THighResTime);
begin
...
var
  ...
  i: Integer;
begin
  LBarrier := MakeBarrier(AThreadCount);
  LOperations := 0;
  ...
  begin
    LThreads[i] := TWorkerThread.Create(LBarrier, @LOperations, Int64(ADurationMs) * 1000000, LStartTime);
    LThreads[i].Start;
  end;
...
const
  DURATION_MS = 1000;
  THREAD_COUNTS: array[0..4] of Integer = (2, 4, 8, 16, 32);
var
  i: Integer;
  LResult: TBenchmarkResult;
  LResults: TBenchmarkResults;
begin
  PrintHeader('Barrier Performance Benchmark');
  SetLength(LResults, 0);
  
  WriteLn('--- Barrier Synchronization (Multiple Threads) ---');
  for i := 0 to High(THREAD_COUNTS) do
  begin
    LResult := RunBenchmark('Barrier/Sync', THREAD_COUNTS[i], DURATION_MS);
    PrintResult(LResult);
  ...
  SaveResultsToCSV(LResults, 'barrier_benchmark_results.csv');
  SaveResultsToJSON(LResults, 'barrier_benchmark_results.json');
  WriteLn;
  WriteLn('Results saved to:');
  WriteLn('  - barrier_benchmark_results.csv');
  WriteLn('  - barrier_benchmark_results.json');
end;
