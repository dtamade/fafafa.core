...
begin
  ...
  // 创建事件
  LEvent := CreateManualResetNamedEvent('StressTest_' + ATestName, False);
...
begin
  ...
  // 高并发中等时间测试
  LResult := RunStressTest(50, 60, 'High_Concurrency_Medium');
  PrintStressResult(LResult);
  
  // 极高并发短时间测试
  LResult := RunStressTest(100, 30, 'Very_High_Concurrency_Short');
  PrintStressResult(LResult);
  
  // 中等并发长时间测试
  LResult := RunStressTest(20, 300, 'Medium_Concurrency_Long');
  PrintStressResult(LResult);
